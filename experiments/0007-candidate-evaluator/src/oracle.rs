//! The deterministic external-state task oracle (Experiment 0007).
//!
...
//! condition name, system prompt, candidate identity, mutation text,
//! trajectory tags, cost, or the model's final-answer wording. The
//! evaluator function signature is:
//!
//! ```ignore
//! fn evaluate_task(task: &TaskSpec, outcome: &OracleInput) -> TaskEvaluation
//! ```
//!
...
/// A task split. Calibration evidence is reported but does not determine
/// the experiment verdict; held-out evidence does.
...
impl TaskSplit {
    pub fn as_str(self) -> &'static str {
        match self {
            TaskSplit::Calibration => "calibration",
            TaskSplit::HeldOut => "heldout",
        }
    }
}
...
/// function accepts. There is no field for condition, prompt, candidate
/// name, or any other experiment-side identity.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
mod tests {
    ...
    fn evaluation_is_independent_of_how_the_state_was_reached() {
        // Anti-circularity: the oracle API cannot receive a condition, so
        // the same (task, termination, initial, final) input must yield
        // the same verdict no matter which condition produced it. The
        // same final state is "produced" here by two different (unseen)
        // trajectories; the input contains nothing that differs.
        let t = task();
