...
//! plus the Experiment 0007 deterministic fault-injection environment.
//!
...
//! episodes.
//!
//! Fault injection (Experiment 0007 addition): exactly two modes,
//! `Reliable` and `DropFirstWrite { key }`. The `DropFirstWrite` mode
//! deterministically drops the first write to the registered key but
//! returns the *same* model-visible success response. The drop is
//! invisible to the model (it can only discover it by reading state) and
//! is recorded in an experiment-only `WriteAttempt` audit log that never
...
/// Registered experiment fault semantics. Exactly two modes.
///
/// Serializes as `{"mode": "reliable"}` or
/// `{"mode": "drop_first_write", "key": "x"}`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "mode", rename_all = "snake_case")]
pub enum FaultMode {
    /// Every valid `state_write` updates external state immediately.
    Reliable,
    /// The FIRST write to the registered key is silently dropped
    /// (one-shot); subsequent writes to that key apply normally.
    DropFirstWrite { key: String },
}

impl FaultMode {
    pub fn is_reliable(&self) -> bool {
        matches!(self, FaultMode::Reliable)
    }

    /// The registered target key for `DropFirstWrite`, if the mode has
    /// one and it is a valid state key.
    pub fn target_key(&self) -> Option<StateKey> {
        match self {
            FaultMode::Reliable => None,
            FaultMode::DropFirstWrite { key } => StateKey::parse(key),
        }
    }
}
...
/// schedule can be proven to have executed as registered. This record is
/// NEVER inserted into the LLM transcript.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct WriteAttempt {
    ...
    pub applied: bool,
    /// `Some("drop_first_write")` when a fault caused the drop, else
    /// `None`.
...
pub struct ToolExecutor {
    state: ExternalState,
    fault: FaultMode,
    /// Whether the one-shot drop for the registered key is still pending.
    drop_pending: bool,
...
impl ToolExecutor {
    ...
    pub fn fresh(initial: &Value, fault: FaultMode) -> Self {
        Self {
            ...
            drop_pending: fault.target_key().is_some(),
            fault,
    ...
    fn write(&mut self, args: &Value) -> Result<ToolResult, ToolError> {
        ...
        // Fault semantics: the drop is silent and one-shot.
        let (applied, fault_reason) = match &self.fault {
            FaultMode::Reliable => (true, None),
            FaultMode::DropFirstWrite { key: fk } if self.drop_pending && fk == key.as_str() => {
                self.drop_pending = false;
                (false, Some("drop_first_write".to_string()))
            }
            _ => (true, None),
        };
...
mod tests {
    ...
    #[test]
    fn drop_first_write_is_silent_and_one_shot() {
        let mut exec = ToolExecutor::fresh(
            &json!({"x": "EMPTY", "y": "B"}),
            FaultMode::DropFirstWrite { key: "x".into() },
        );

        // First write to the registered key: model sees success, state
        // is NOT changed, the drop is audited.
        let r1 = exec
            .execute("state_write", &json!({"key": "x", "value": "A"}))
            .unwrap();
        assert_eq!(exec.final_state_value()["x"], "EMPTY");
        assert_eq!(
            ...
            WriteAttempt {
                ...
                applied: false,
                fault_reason: Some("drop_first_write".into()),
            }
        );

        // Second write: applies normally.
        let r2 = exec
            .execute("state_write", &json!({"key": "x", "value": "A"}))
            .unwrap();
        assert_eq!(exec.final_state_value()["x"], "A");
        assert!(exec.write_attempts()[1].applied);
        assert_eq!(exec.write_attempts()[1].fault_reason, None);

        // Model-visible payloads are byte-identical across the two cases:
        // the fault is invisible to the model.
