//! Bounded prompt-mutation generation (Experiment 0010-specific).
//!
//! The ONLY evolvable surface in this experiment is an append-only
//! system-prompt policy suffix (spec §15). A candidate is:
//!
//! ```text
...
//! execution (spec §8): it receives a strict whitelist
//! [`MutationInput`] packet built from discovery evidence, produces
//! exactly four suffixes + rationales in JSON, and nothing else. The
//! model receives no tools, writes no files, and cannot influence the
//! evaluator, the selection authority, or the oracle (spec §59/§61).
//!
...
use crate::trace::{DiscoveryRecord, EXPERIMENT_ID, PROMOTION_TASKS, SELECTION_TASKS};

/// Registered suffix length bounds (spec §17).
pub const MIN_SUFFIX_WORDS: usize = 5;
pub const MAX_SUFFIX_WORDS: usize = 100;
pub const MAX_SUFFIX_BYTES: usize = 800;

/// Registered candidate count (spec §16).
pub const CANDIDATE_COUNT: usize = 4;

/// Maximum number of full mutation-generation attempts (spec §27).
pub const MAX_GENERATION_ATTEMPTS: u32 = 2;

/// Candidate IDs, assigned by the harness in returned array order
/// (spec §16). The model does not choose IDs.
pub const CANDIDATE_IDS: [&str; CANDIDATE_COUNT] = ["C1", "C2", "C3", "C4"];
...
/// Terms a candidate suffix must NOT contain (spec §17). Compared
/// case-insensitively. "state_read"/"state_write" ARE allowed (visible
/// tools).
pub const FORBIDDEN_TERMS: [&str; 15] = [
    "experiment",
    "benchmark",
    "oracle",
    "promotion",
...
pub struct CandidateEntry {
    ...
    /// Experiment-only lineage: generation 1, parent G0 (spec §29).
    pub generation: u32,
    ...
    /// Stored for audit only. MUST NOT affect selection (spec §26/§62).
    pub rationale: String,
    pub word_count: u32,
    pub byte_count: u32,
}

/// The frozen candidate pool (spec §30). Once committed, the content
/// must never change.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct CandidatePool {
    ...
    pub mutation_input_sha256: String,
    /// Exactly four entries, in harness-assigned order C1..C4.
...
/// always re-derivable from the two frozen inputs (spec §66).
...
// ===========================================================================
// Suffix content validation (spec §17/§66)
// ===========================================================================

/// Machine-check for synthetic state literals `V_[A-Z0-9_]+`
/// (spec §18). Returns true when any occurrence exists.
pub fn contains_v_literal(s: &str) -> bool {
    let bytes = s.as_bytes();
    let mut i = 0;
    while i + 2 <= bytes.len() {
        if bytes[i] == b'V' && bytes[i + 1] == b'_' {
            let mut j = i + 2;
            while j < bytes.len()
...
/// The registry is used for the registered-task-ID check (spec §17).
pub fn suffix_content_errors(suffix: &str, registry: &[TaskSpec]) -> Vec<String> {
    ...
    if contains_v_literal(suffix) {
        errors.push("suffix contains a synthetic V_* state literal".to_string());
    }
    // Registered task IDs must never appear in a policy suffix (spec §17):
...
/// Validate one candidate suffix: content bounds (spec §17).
pub fn validate_suffix(suffix: &str, registry: &[TaskSpec]) -> Vec<String> {
    suffix_content_errors(suffix, registry)
}

/// Full pool validation against spec §66. `incumbent` is the incumbent
/// prompt text (file content); `incumbent_file_sha256` its byte hash.
pub fn validate_candidate_pool(
    ...
    for (i, c) in pool.candidates.iter().enumerate() {
        ...
        if i < CANDIDATE_COUNT && c.candidate_id != CANDIDATE_IDS[i] {
            errors.push(format!(
                "candidate[{i}] has id {:?}, expected {:?}",
                c.candidate_id, CANDIDATE_IDS[i]
            ));
        }
...
// ===========================================================================
// Mutation input packet (strict whitelist, spec §23)
// ===========================================================================
...
/// The complete mutation-input packet (spec §23).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
/// failures are excluded (spec §22): their episodes are not behavioral
/// evidence.
...
/// Machine-level leakage verification of a mutation input (spec §24).
...
pub fn mutation_input_leak_errors(input: &MutationInput, registry: &[TaskSpec]) -> Vec<String> {
    ...
    // Selection / promotion task IDs.
    for id in SELECTION_TASKS.iter().chain(PROMOTION_TASKS.iter()) {
        if text.contains(id) {
            errors.push(format!("mutation input contains {id:?} task reference"));
        }
    }
...
// ===========================================================================
// Mutator response parsing + structural validation (spec §26/§27)
// ===========================================================================
...
/// Parse + structurally validate ONE mutator response
/// (spec §26/§27). Returns `(accepted_candidates, errors)`; `errors`
/// empty ⇒ fully valid pool. The second generation attempt receives
/// these machine-generated errors verbatim (spec §27).
pub fn validate_mutator_response(
    ...
    let output: RawMutatorOutput = match serde_json::from_value(parsed.clone()) {
        Ok(o) => o,
        Err(e) => {
            return (
                None,
                vec![format!(
                    "wrong schema (expected {{\"candidates\": [...]}}): {e}"
                )],
            );
        }
    };
    let mut errors = Vec::new();
    if output.candidates.len() != CANDIDATE_COUNT {
        errors.push(format!(
            "wrong candidate count: {} (expected exactly {CANDIDATE_COUNT})",
    ...
    if errors.is_empty() {
        (Some(output.candidates), Vec::new())
    } else {
        (None, errors)
    }
}
...
/// assigns C1..C4 in returned array order (spec §26).
pub fn build_pool(
    ...
    let candidates = candidates
        ...
        .map(|(i, c)| CandidateEntry {
            candidate_id: CANDIDATE_IDS[i].to_string(),
...
/// The complete mutation-generation artifact (spec §28).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct GenerationArtifact {
    ...
    /// Accepted candidate pool (spec §27: the FIRST fully-valid pool).
...
mod tests {
    ...
    fn valid_pool() -> CandidatePool {
        ...
        build_pool(&cands, incumbents, &"3".repeat(64), &"2".repeat(64))
    }
    ...
    #[test]
    fn v_literal_detection() {
        assert!(contains_v_literal("use V_DA_0 here"));
        assert!(contains_v_literal("V_X"));
        assert!(!contains_v_literal("no literals"));
        assert!(!contains_v_literal("the word V_ alone"));
    }
    ...
    fn forbidden_terms_and_task_ids_rejected() {
        ...
        for bad in [
            ...
            "Mind the stress level of every state operation you make.",
            "Track S2 counters while you observe each state key once.",
            "Read fault_reason after each write to the current state.",
            "Remember registered_drop_index after each state write occurs.",
            "Set V_P1_1 whenever the state appears to need a change now.",
        ] {
    ...
    fn pool_validation_rejects_bad_pools() {
        ...
        let pool = valid_pool();
        assert!(
            validate_candidate_pool(&pool, incumbents, &"1".repeat(64), &reg)
                .iter()
        ...
        assert!(validate_candidate_pool(&p2, incumbents, &"1".repeat(64), &reg).is_empty());
        // Tamper a suffix → hash mismatch + possibly content errors.
        let mut p3 = p2.clone();
        p3.candidates[0].suffix = "TAMPERED SUFFIX".to_string();
        assert!(
            validate_candidate_pool(&p3, incumbents, &"1".repeat(64), &reg)
                .iter()
        ...
        let mut p4 = p2.clone();
        p4.candidates.pop();
        let e4 = validate_candidate_pool(&p4, incumbents, &"1".repeat(64), &reg);
        ...
        assert!(
            validate_candidate_pool(&p5, incumbents, &"1".repeat(64), &reg)
                .iter()
    ...
    fn mutator_response_validation_counts_and_fences() {
        ...
        let (c, e) = validate_mutator_response(&raw.to_string(), incumbents, &reg);
        assert!(c.is_none());
        assert!(e.iter().any(|x| x.contains("count")), "{e:?}");
        // Fences are stripped deterministically (including the trailing
        // newline between the closing fence and the end of the message).
        let body = r#"{"candidates":[{"suffix":"a b c d e f"},{"suffix":"g h i j k l"},{"suffix":"m n o p q r"},{"suffix":"s t u v w x"}]}"#;
        let (c, e) = validate_mutator_response(&format!("```json\n{body}\n```"), incumbents, &reg);
        assert!(c.is_some() && e.is_empty(), "{e:?}");
        // A rationale-less candidate is still parsed (the field is
        // optional on the wire; it is never part of the evolution surface).
        let raw = json!({"candidates":[{"suffix":"a b c d e f"}]});
        let (c, e) = validate_mutator_response(&raw.to_string(), incumbents, &reg);
        assert!(c.is_none(), "{e:?}"); // count is still wrong
    }
    ...
    fn mutation_input_whitelist_and_leak_check() {
        ...
        let sel = reg.iter().find(|t| t.id == "SEL1").unwrap();
        input.discovery_episodes.push(MutationInputEpisode {
            ...
            final_state: json!({"x": "V_D1_0", "y": "V_KEEP_1"}),
            termination_reason: "completed".into(),
            oracle_success: false,
        });
        let errs = mutation_input_leak_errors(&input, &reg);
        assert!(errs.iter().any(|e| e.contains("V_S1_0")), "{errs:?}");
        // A selection task ID in the input is a hard leak.
        let mut input2 = input.clone();
        input2.discovery_episodes[0].task_id = "SEL1".into();
        let errs2 = mutation_input_leak_errors(&input2, &reg);
        assert!(errs2.iter().any(|e| e.contains("SEL1")), "{errs2:?}");
    }

    #[test]
    fn known_repair_text_is_structurally_valid_but_not_injectable() {
        ...
        // text is banned. This test pins that the text would pass
        // structural checks, so the raw-response binding is the guard.
