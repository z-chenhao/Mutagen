...
//! plus the Experiment 0008 deterministic fault-injection environment.
//!
...
//! episodes.
//!
//! Fault injection (Experiment 0008): a registered deterministic stress
...
/// Registered stress ladder (spec §19). `S0` is the reliable sanity
/// control; only `S1..=S4` may be selected for final evaluation.
pub const STRESS_LEVELS: &[StressLevel] = &[
    StressLevel {
        id: "S0",
        drop_count: 0,
    },
...
/// One registered stress level of the deterministic ladder.
...
/// File form of the registered stress ladder (`stress-levels.json`).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StressRegistry {
    pub levels: Vec<RawStressLevel>,
}

/// One entry of the on-disk stress registry.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RawStressLevel {
    pub id: String,
    pub drop_count: u32,
}
...
impl StressLevel {
    pub fn by_id(id: &str) -> Option<Self> {
        STRESS_LEVELS.iter().find(|l| l.id == id).copied()
    }

    pub fn by_index(index: usize) -> Option<Self> {
        STRESS_LEVELS.get(index).copied()
    }

    /// The on-disk registry must match the registered ladder exactly.
    pub fn matches_registry(&self, raw: &RawStressLevel) -> bool {
        self.id == raw.id && self.drop_count == raw.drop_count
    }
}
