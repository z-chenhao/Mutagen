//! Bounded prompt-mutation generation (Experiment 0013-specific).
//!
...
//!
//! Anti-seeding (0013 critical requirement): no candidate from
...
/// Maximum number of full mutation-generation attempts (0013: the
...
/// Terms a candidate suffix must NOT contain (0013 spec §16). Compared
/// case-insensitively against the lower-cased suffix. "state_read"/
/// "state_write" ARE allowed (visible tools). Separately, a suffix
/// must not contain any registered task ID or any `V13_*` literal.
...
/// Machine-check for the 0013 synthetic state literal pattern
/// `V13_[A-Z0-9_]+`. Returns true when any occurrence exists.
pub fn contains_v13_literal(s: &str) -> bool {
    let needle = b"V13_";
...
pub fn suffix_content_errors(suffix: &str, registry: &[TaskSpec]) -> Vec<String> {
    ...
    if contains_v13_literal(suffix) {
        errors.push("suffix contains a synthetic V13_* state literal".to_string());
    }
...
/// The shape-informative retry message for an invalid attempt (0013
...
pub fn validate_mutator_response(
    ...
    // --- structural validation (the 0013 explicit schema) -------------
    let mut errors = Vec::new();
...
mod tests {
    ...
    fn design() -> Design {
        crate::experiment::load_design().expect("0013 design manifest loads and validates")
    }
    ...
    #[test]
    fn v13_literal_detection() {
        assert!(contains_v13_literal("use V13_DA_0 here"));
        assert!(contains_v13_literal("V13_X"));
        assert!(!contains_v13_literal("no literals"));
        assert!(!contains_v13_literal("the word V13_ alone"));
        assert!(!contains_v13_literal(
            "old V_D1_0 literal is not a 0013 literal"
        ));
    }
    ...
    fn forbidden_terms_and_task_ids_rejected() {
        ...
        for bad in [
            ...
            "Set V13_PA_1 whenever the state appears to need a change now.",
        ] {
    ...
    fn frozen_prompt_documents_the_explicit_schema() {
        // The 0013 fix: the frozen mutator prompt must state the exact
    ...
    fn mutation_input_whitelist_and_leak_check() {
        ...
        let sel = reg.iter().find(|t| t.id == "S13A").unwrap();
