...
//! Experiment 0013 (the frozen end-to-end self-evolution
//! confirmation).
//!
...
/// The registered deadline contract (0013 single source of truth).
///
...
pub struct Design {
    ...
    /// and the return/teardown tolerance (0013 deadline-safety).
    ...
    /// fallback model. The endpoint URL is a registered named constant
    /// in `src/model.rs` (it is not a count or a gate, so it is not a
    /// design field); the per-episode time budget IS a design field
    /// (the `deadline` block above).
    pub model: String,
}
...
impl Design {
    ...
    /// wrapped — the 0013 registered schedule has five conditions, so
...
pub struct RunIdentity {
    /// The registered run identity (design `run_id`), e.g. "0013-r1".
    pub run_id: String,
...
pub struct ModelRequestRecord {
    ...
    // --- 0013 request-level deadline evidence (mandatory on every real
...
pub struct CostBlock {
    ...
    pub model_wait_time_ms: u64,
    pub tool_execution_time_us: u64,
    // --- 0013 deadline summary metrics (re-derived from the
...
/// The 0013 registered request-level deadline contract (spec §28).
///
...
fn verify_record_core<R: RecordCoreLike>(
    ...
    // 0013: the request-level deadline contract is the authoritative
...
pub fn build_synthetic_artifacts() -> SyntheticArtifacts {
    ...
    let design = Design::load(&dir.join("design.json")).expect("0013 design manifest loads");
