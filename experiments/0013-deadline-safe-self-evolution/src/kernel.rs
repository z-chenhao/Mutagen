...
//!
//! Experiment 0013: the hard limits (`max_model_turns`,
...
//!
//! 0013 deadline contract (the experiment's only semantic delta, spec
...
//!
//! Experiment 0013 (deadline-safe semantics): the kernel OWNS the
...
use crate::trace::{
    DesignDeadline, ExecutedToolCall, ModelRequestRecord, RequestedToolCall, TerminationReason,
    Timing,
};
...
/// episode-deadline constant exists anywhere else — 0013 rule).
...
// Deadline-aware model call protocol (0013, spec §21/§22): the
...
/// real run, a scripted closure in network-free tests. It receives the
/// exact integer per-request timeout (ms) the kernel registered for
/// this request — `min(remaining episode budget, registered ceiling)` —
/// and the kernel measures the start/finish instants itself against the
...
pub fn run_episode(
    mut call_model: impl FnMut(&[Message], &[ToolSpec], u64) -> Result<ModelResponse, ModelError>,
...
mod tests {
    ...
    // --- 0013 deadline-semantics kernel tests (network-free) --------
