...
| Closed typed event union with agent/turn/message/tool lifecycles | Structured, lifecycle-ordered events give an authoritative trajectory | Small closed `TrajectoryEvent` enum (8 variants); the kernel itself appends events to `EpisodeState.events` — trajectory is never reconstructed from logs | adopted |
