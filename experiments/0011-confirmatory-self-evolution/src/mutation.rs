//! Bounded prompt-mutation generation (Experiment 0011-specific).
//!
...
//!
//! Anti-seeding (0011 critical requirement): no prior candidate from
//! Experiment 0010 is copied, hard-coded, hinted, or seeded anywhere in
//! this crate. The mutator prompt is neutral; if the fresh mutator
//! independently rediscoveries a semantically similar policy, that is
//! ACCEPTED as independent rediscovery — never rejected post hoc.
...
/// Maximum number of full mutation-generation attempts.
...
/// Terms a candidate suffix must NOT contain. Compared
...
/// Machine-check for the 0011 synthetic state literal pattern
/// `V11_[A-Z0-9_]+`. Returns true when any occurrence exists.
pub fn contains_v11_literal(s: &str) -> bool {
    let needle = b"V11_";
...
pub fn suffix_content_errors(suffix: &str, registry: &[TaskSpec]) -> Vec<String> {
    ...
    if contains_v11_literal(suffix) {
        errors.push("suffix contains a synthetic V11_* state literal".to_string());
    }
...
/// valid pool. The second generation attempt receives these
/// machine-generated errors verbatim (no performance or selection
/// feedback).
...
mod tests {
    ...
    fn design() -> Design {
        crate::experiment::load_design().expect("0011 design manifest loads and validates")
    }
    ...
    #[test]
    fn v11_literal_detection() {
        assert!(contains_v11_literal("use V11_DA_0 here"));
        assert!(contains_v11_literal("V11_X"));
        assert!(!contains_v11_literal("no literals"));
        assert!(!contains_v11_literal("the word V11_ alone"));
        assert!(!contains_v11_literal(
            "old V_D1_0 literal is not a 0011 literal"
        ));
    }
    ...
    fn forbidden_terms_and_task_ids_rejected() {
        ...
        for bad in [
            ...
            "Set V11_PA_1 whenever the state appears to need a change now.",
        ] {
    ...
    fn mutation_input_whitelist_and_leak_check() {
        ...
        let sel = reg.iter().find(|t| t.id == "S11A").unwrap();
