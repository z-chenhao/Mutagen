//! Pre-registered statistics for Experiment 0009, carried over into
//! 0011 with one change: the significance level is a *parameter*
//! (from the design manifest `promotion.alpha`) rather than a constant
//! in this crate.
//!
...
///
/// Experiment 0009 sizes (n ≤ 48 non-tied pairs at most) are far below
/// any floating-point precision concern: every C(n, k) for n ≤ 64 is
/// exact in an `f64` mantissa.
...
/// at the design-registered significance level.
///
/// - `quality_improvement`: wins > losses AND p ≤ alpha
/// - `quality_regression`:  losses > wins AND p ≤ alpha
/// - otherwise: `quality_inconclusive` (never "neutral")
pub fn classify_quality(wins: u64, losses: u64, alpha: f64) -> QualityClass {
...
mod tests {
    ...
    fn classification_improvement() {
        ...
        assert_eq!(classify_quality(9, 1, 0.05), QualityClass::Improvement);
        // 6 wins 0 losses, n=6: p = 2/64 = 0.03125 ≤ 0.05.
        assert_eq!(classify_quality(6, 0, 0.05), QualityClass::Improvement);
    }

    #[test]
    fn classification_regression() {
        assert_eq!(classify_quality(1, 9, 0.05), QualityClass::Regression);
        assert_eq!(classify_quality(0, 6, 0.05), QualityClass::Regression);
    }

    #[test]
    fn classification_inconclusive() {
        // Balanced: never "neutral", only "inconclusive".
        assert_eq!(classify_quality(5, 5, 0.05), QualityClass::Inconclusive);
        // Directional but not significant: 4 wins 3 losses, n=7:
        // p = 2 * (1 + 7 + 21) / 2^7 = 58/128 = 0.4531 > 0.05.
        assert_eq!(classify_quality(4, 3, 0.05), QualityClass::Inconclusive);
        // No non-tied pairs.
        assert_eq!(classify_quality(0, 0, 0.05), QualityClass::Inconclusive);
    }
