...
//! Experiment 0011 (the clean confirmatory self-evolution rerun).
//!
...
/// One registered model endpoint (the frozen local endpoint; the
/// mutation generator reuses the same underlying model via
/// `mutator.endpoint_index`).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct DesignEndpoint {
    pub url: String,
    pub model: String,
    /// Whether the request carries an Authorization header.
    #[serde(default)]
    pub auth_header: bool,
}

/// The mutation generator's model binding: the SAME underlying model
/// as agent execution, selected by endpoint index (spec: "the same
/// underlying model used for agent execution").
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct DesignMutator {
    pub model: String,
...
pub struct Design {
    ...
    /// Hard per-episode time limit in milliseconds (spec: 10 minutes).
    pub max_time_ms: u64,
    /// The registered model endpoint(s).
    pub endpoints: Vec<DesignEndpoint>,
    /// The mutation generator binding (same underlying model).
    pub mutator: DesignMutator,
}
...
impl Design {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.max_time_ms < 60_000 || self.max_time_ms > 3_600_000 {
            errors.push(format!(
                "max_time_ms {} outside the sane window 60_000..=3_600_000",
                self.max_time_ms
            ));
        }
        if self.endpoints.is_empty() {
            errors.push("endpoints must be non-empty".to_string());
        }
        for (i, e) in self.endpoints.iter().enumerate() {
            if e.url.trim().is_empty() || e.model.trim().is_empty() {
                errors.push(format!("endpoint[{i}] url/model must be non-empty"));
            }
        }
        if self.mutator.endpoint_index >= self.endpoints.len() {
            errors.push(format!(
                "mutator.endpoint_index {} is out of range ({} endpoints)",
                self.mutator.endpoint_index,
                self.endpoints.len()
            ));
        } else if self.mutator.model != self.endpoints[self.mutator.endpoint_index].model {
            errors.push(
                "the mutation generator must use the same underlying model as its endpoint"
                    .to_string(),
            );
        }
        // Discovery gates.
    ...
    /// The registered endpoint for the mutation generator.
    pub fn mutator_endpoint(&self) -> &DesignEndpoint {
        &self.endpoints[self.mutator.endpoint_index]
    }
...
pub struct RunIdentity {
    /// The registered run identity (design `run_id`), e.g. "0011-r1".
    pub run_id: String,
...
fn verify_record_core<R: RecordCoreLike>(
    ...
    if record.timing().wall_time_ms > design.max_time_ms {
        errors.push(format!(
            "{who}: wall time {} ms exceeds the registered {} ms episode time limit",
...
pub fn build_synthetic_artifacts() -> SyntheticArtifacts {
    ...
    let design = Design::load(&dir.join("design.json")).expect("0011 design manifest loads");
