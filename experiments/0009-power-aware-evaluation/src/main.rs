//! Experiment 0009 runner: power-aware, candidate-blind, family-level
//! difficulty-calibrated improvement detection.
//!
//! Usage:
//!   self-test
//!   calibrate --trajectories PATH --summary PATH --plan PATH [--code-commit SHA] [--endpoint REDACTED]
//!   evaluate  --plan PATH --trajectories PATH --summary PATH [--code-commit SHA] [--endpoint REDACTED]
//!   verify-calibration --trajectories PATH --summary PATH --plan PATH
//!   verify-evaluation  --plan PATH --trajectories PATH --summary PATH
//!
...
//! `--code-commit` defaults to the current `git rev-parse HEAD`.
//!
...
fn cmd_calibrate(flags: Flags) -> Result<(), String> {
    let config = experiment::CalibrateConfig {
        ...
        plan: flags.plan.ok_or("calibrate requires --plan")?,
...
fn cmd_evaluate(flags: Flags) -> Result<(), String> {
    let config = experiment::EvaluateConfig {
        plan: flags.plan.ok_or("evaluate requires --plan")?,
        trajectories: flags
            .trajectories
...
fn cmd_verify_calibration(flags: Flags) -> Result<(), String> {
    ...
    let plan_path = flags.plan.ok_or("verify-calibration requires --plan")?;

    let record_values = read_lines(&traj_path)?;
    let summary_value = read_json(&summary_path, "summary")?;
    let plan_value = read_json(&plan_path, "evaluation plan")?;
...
fn cmd_verify_evaluation(flags: Flags) -> Result<(), String> {
    let plan_path = flags.plan.ok_or("verify-evaluation requires --plan")?;
    let traj_path = flags
        .trajectories
    ...
    let task_hash = experiment::task_registry_sha256(&registry)
        .map_err(|e| format!("cannot hash task registry: {e}"))?;
    let plan_bytes =
        std::fs::read(&plan_path).map_err(|e| format!("cannot read evaluation plan: {e}"))?;
    let plan_file_sha = crate::protocol::sha256_bytes(&plan_bytes);
...
fn main() -> ExitCode {
    ...
    let outcome = match cmd {
        ...
        "calibrate" => parse_flags(
            &rest,
            &["--trajectories", "--summary", "--plan"],
        )
        .and_then(cmd_calibrate),
        "evaluate" => parse_flags(&rest, &["--plan", "--trajectories", "--summary"])
        ...
        "verify-evaluation" => {
            parse_flags(&rest, &["--plan", "--trajectories", "--summary"])
                .and_then(cmd_verify_evaluation)
        }
        _ => Err(
            "usage: mutagen-exp-0009 <self-test | calibrate --trajectories P --summary P --plan P [--code-commit SHA] [--endpoint REDACTED] | evaluate --plan P --trajectories P --summary P [--code-commit SHA] [--endpoint REDACTED] | verify-calibration --trajectories P --summary P --plan P | verify-evaluation --plan P --trajectories P --summary P>"
                .to_string(),
        ),
    };
...
mod tests {
    ...
    fn parser_accepts_flags() {
        ...
        assert_eq!(flags.plan.as_deref(), Some(std::path::Path::new("p.json")));
