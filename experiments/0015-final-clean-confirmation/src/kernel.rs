//! The experimental agent kernel (Rust-owned turn loop), carried over unchanged from 0006–0011.
//!
//! Carried over from Experiment 0006, with the Experiment 0008
//! usage and audit recording (identical semantics through 0009/0010):
...
//! - fault-injecting tool environment with the repeated silent-drop
//!   ladder (see `tools.rs`).
//!
//! Experiment 0010/0011 addition over the carried-over kernel: the
//! `EpisodeOutcome` also returns the model-visible transcript
//! (`conversation`) — the exact message list the kernel sent to the
//! model, in order. The turn loop, limits, and failure semantics are
//! otherwise identical to the carried-over kernel. The transcript is
//! what the mutation generator is allowed to see (spec §23); it
//! contains no hidden audit fields by construction (`Message` and
//! `ToolCall` declare only the wire-visible fields).
//!
//! Experiment 0015: the hard limits (`max_model_turns`,
//! `max_tool_calls`) are read from the single machine-readable design
//! authority (`design.json`) via `KernelConfig::from_design` — they are
//! no longer a second, independently hard-coded source of truth.
//!
//! 0015 deadline contract (the experiment's only semantic delta, spec
//! §19-§22): the same `KernelConfig` carries the registered deadline
//! contract (`Design::deadline`): `episode_deadline_ms`,
//! `request_timeout_ceiling_ms`, `deadline_return_tolerance_ms`. The
//! kernel enforces it: no request may start after the deadline;
//! every request is issued with the exact registered integer timeout
//! `min(remaining episode budget, ceiling)`; a response that returns
//! after the deadline is discarded (never pushed into the transcript,
//! never a tool call, never the final answer, no subsequent request);
//! every request record carries the request-level timing evidence the
//! artifact verifier re-derives validity from.
//!
//! Experiment 0015 (deadline-safe semantics): the kernel OWNS the
//! registered deadline contract (`design.json` `deadline` block via
//! `KernelConfig.deadline`). For every model request it enforces, with
//! monotonic `std::time::Instant` elapsed time relative to the episode
//! start (never wall-clock timestamps):
//!
//! - **Request start**: if `start >= deadline` the request is NOT made
//!   and the episode terminates `episode_deadline_reached`
//!   (infrastructure failure, no request record emitted).
//! - **Requested timeout**: `min(remaining_budget, ceiling)`, always
//!   `> 0`, passed to the model call together with the measured
//!   evidence as a [`TimedModelCall`]; the exact integer is recorded.
//! - **Response acceptance**: a response that returns at or before the
//!   deadline is accepted; a response that returns AFTER the deadline
//!   (even inside the registered return/teardown tolerance) is
//!   DISCARDED — it produces no assistant message, no tool call, and no
//!   final answer — and the episode terminates
//!   `episode_deadline_exceeded` (a VALID infrastructure failure, not a
//!   protocol integrity violation). The tolerance is an observation /
//!   teardown bound, never additional productive agent time.
//!
//! The same timing evidence the kernel used for control flow is
//! persisted into every `ModelRequestRecord` (`trace.rs`), and the
//! verifier (`deadline_evidence_violations`) re-derives the contract
//! from the artifacts. No response content of a discarded response is
//! persisted: only the classified result, not the model text.
//!
...
use crate::trace::{
    ChannelBinding, DesignDeadline, ExecutedToolCall, ModelRequestRecord, RequestedToolCall,
    TerminationReason, Timing,
};

/// Hard experiment limits: identical across all conditions.
/// Limit hits count as agent-caused task failures.
///
/// The values come from the design authority (`design.json`
/// `max_model_turns` / `max_tool_calls` / `deadline`); there is no
/// second source of truth in this crate (in particular, no duplicate
/// episode-deadline constant exists anywhere else — 0015 rule).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct KernelConfig {
    pub max_model_turns: u32,
    pub max_tool_calls: u32,
    /// The registered deadline contract (`design.json` `deadline`).
    pub deadline: DesignDeadline,
}

impl KernelConfig {
    /// Build the frozen kernel limits from the design manifest.
    pub fn from_design(design: &crate::trace::Design) -> Self {
        Self {
            max_model_turns: design.max_model_turns,
            max_tool_calls: design.max_tool_calls,
...
pub struct EpisodeOutcome {
    ...
    /// The model-visible transcript: the exact message list sent to the
    /// model (system, user, assistant/tool messages), in order.
...
fn empty_request(turn: u32) -> ModelRequestRecord {
    ModelRequestRecord {
        ...
        // The audited-channel binding (0015) is stamped from the
        // model-boundary return immediately after each call.
        channel_request_id: String::new(),
        request_body_sha256: String::new(),
    }
}

// Deadline-aware model call protocol (0015, spec §21/§22): the
// kernel wraps every model request. It measures the request start and
// finish itself, computes the exact registered per-request timeout
// (`min(remaining episode budget, deadline ceiling)` in ms) from the
// design-registered deadline contract, and passes THAT value to the
// model-call closure. From the same evidence the kernel makes the
// response-acceptance decision: a response that returned after the
// productive episode deadline is discarded — never pushed into the
// transcript, never counted as a model turn, never executed as a tool
// call, never recorded as a final answer — and no subsequent request
// may be made. The trajectory persists exactly the same evidence in
// the request records; the artifact verifier re-derives validity from
// it.
...
/// failure.
///
/// `call_model` is the model boundary: the registered channel path
/// (`ModelClient::send` through the audited gateway) in the real run,
/// a scripted closure in network-free tests. It receives the exact
/// integer per-request timeout (ms) the kernel registered for this
/// request — `min(remaining episode budget, registered ceiling)` — and
/// the kernel measures the start/finish instants itself against the
/// episode start (`std::time::Instant`, monotonic). `tools` must be a
/// *fresh* `ToolExecutor` per episode so no external state or fault
/// budget is reused between episodes.
///
/// 0015: the boundary also returns the audited-channel binding
/// ([`ChannelBinding`]: the channel-issued request identity plus the
/// hash of the exact body bytes that traversed the gateway), which the
/// kernel stamps into the request record. Network-free scripted
/// closures return an empty binding. The turn loop, limits, deadline
/// enforcement, and response-acceptance semantics are unchanged from
/// 0013.
pub fn run_episode(
    mut call_model: impl FnMut(
        &[Message],
        &[ToolSpec],
        u64,
    ) -> Result<(ModelResponse, ChannelBinding), ModelError>,
    ...
    let deadline_ms = config.deadline.episode_deadline_ms;
    let timeout_ceiling_ms = config.deadline.request_timeout_ceiling_ms;
    ...
    loop {
        ...
        // --- Request-start contract (registered deadline) -------------
        let start_ms = episode_started.elapsed().as_millis() as u64;
        if start_ms >= deadline_ms {
            // The productive episode deadline elapsed before another
            // request could be started: do NOT call the endpoint, do
            // NOT emit a request record; terminate as a (valid)
            // infrastructure failure.
            termination = TerminationReason::EpisodeDeadlineReached;
            break;
        }

        // --- Requested-timeout contract (registered deadline) ---------
        let remaining_ms = deadline_ms - start_ms; // > 0 (checked above)
        let requested_timeout_ms = remaining_ms.min(timeout_ceiling_ms);
        debug_assert!(requested_timeout_ms > 0);

        // --- The blocking model call, measured by the kernel ----------
        let wait = Instant::now();
        let result = call_model(&messages, specs, requested_timeout_ms);
        model_wait_total += wait.elapsed();
        let finish_ms = episode_started.elapsed().as_millis() as u64;

        let mut request = empty_request(turn);
        request.request_started_elapsed_ms = start_ms;
        request.requested_timeout_ms = requested_timeout_ms;
        request.request_finished_elapsed_ms = finish_ms;
        request.timed_out = matches!(&result, Err(ModelError::Timeout(_)));
        // Response acceptance: a usable response is accepted only when
        // it returned at or before the productive deadline. A response
        // that returns after the deadline — even inside the registered
        // return/teardown tolerance — is discarded and must not
        // influence the trajectory in any way.
        request.response_accepted = match &result {
            Ok(_) => finish_ms <= deadline_ms,
            Err(_) => false,
        };

        let resp = match result {
            Ok(_) if !request.response_accepted => {
                // Deadline crossing: the response is discarded. No
                // assistant message, no tool call, no final answer,
                // no subsequent request.
                model_requests.push(request);
                termination = TerminationReason::EpisodeDeadlineExceeded;
                break;
            }
            Ok((r, binding)) => {
                model_turn_count += 1;
                ...
                request.channel_request_id = binding.request_id;
                request.request_body_sha256 = binding.body_sha256;
                model_requests.push(request);
                r
            }
            Err(e) => {
                ...
                model_requests.push(request);
                // Transport failures (incl. structured timeouts),
                // non-2xx statuses, and malformed provider responses
                // are all infrastructure failures.
                termination = match &e {
                    ModelError::Timeout(_) | ModelError::Http(_) | ModelError::Status(_) => {
    ...
    let total = episode_started.elapsed();
...
/// The kernel-recorded execution core of one episode, independent of
/// any experiment presentation (artifact shape, provenance, prompts).
/// Both the Phase A and Phase B record types are assembled from this.
...
/// Project a kernel outcome into its execution core. The kernel does
/// NOT know which artifact shape, condition, or provenance will be
/// assembled around the core: the runner does.
pub fn core(outcome: EpisodeOutcome) -> RecordCore {
    ...
    RecordCore {
        ...
        write_attempts: outcome.write_attempts,
...
mod tests {
    use super::*;
    use crate::protocol::{Role, ToolCall, parse_response};
    use crate::tools::{FaultMode, ToolExecutor};
    use serde_json::Value;
    use serde_json::json;

    type ModelResult = std::result::Result<(ModelResponse, ChannelBinding), ModelError>;

    /// The registered limits (the 0011 design values) for tests.
    fn limits() -> KernelConfig {
        KernelConfig {
            max_model_turns: 12,
            max_tool_calls: 16,
            deadline: DesignDeadline {
                episode_deadline_ms: 600_000,
                request_timeout_ceiling_ms: 600_000,
                deadline_return_tolerance_ms: 1_000,
            },
        }
    }

    /// A config with a short deadline for deadline-semantics tests.
    fn short_deadline_ms(ms: u64) -> KernelConfig {
        KernelConfig {
            max_model_turns: 12,
            max_tool_calls: 16,
            deadline: DesignDeadline {
                episode_deadline_ms: ms,
                request_timeout_ceiling_ms: ms,
                deadline_return_tolerance_ms: 1_000,
            },
        }
    }
    ...
    fn scripted(responses: &[&str]) -> impl FnMut(&[Message], &[ToolSpec], u64) -> ModelResult {
        let mut i = 0usize;
        move |_messages: &[Message], _tools: &[ToolSpec], _timeout_ms: u64| {
            let raw = responses[i.min(responses.len() - 1)];
            let raw: Value = serde_json::from_str(raw).unwrap();
            i += 1;
            // Network-free scripted model: an empty channel binding.
            parse_response(&raw)
                .map(|r| (r, ChannelBinding::default()))
    ...
    fn run_with<F: FnMut(&[Message], &[ToolSpec], u64) -> ModelResult>(
        call: F,
        fault: FaultMode,
        config: &KernelConfig,
    ) -> EpisodeOutcome {
    ...
    fn final_answer_completes_the_episode() {
        let outcome = run_with(scripted(&[FINAL]), FaultMode::Reliable, &limits());
    ...
    fn per_request_usage_records_nulls_and_cache_detail() {
        let outcome = run_with(scripted(&[USAGE, FINAL]), FaultMode::Reliable, &limits());
    ...
    fn cached_exceeding_prompt_is_an_accounting_error() {
        ...
        let outcome = run_with(scripted(&[bad]), FaultMode::Reliable, &limits());
    ...
    fn turn_limit_counts_as_agent_failure() {
        let outcome = run_with(scripted(&[READ_X]), FaultMode::Reliable, &limits());
    ...
    fn parallel_tool_calls_fail_and_record_requested_but_execute_none() {
        let outcome = run_with(scripted(&[PARALLEL]), FaultMode::Reliable, &limits());
        assert_eq!(
    ...
    fn empty_response_is_no_final_answer_agent_failure() {
        ...
        let outcome = run_with(scripted(&[empty]), FaultMode::Reliable, &limits());
    ...
    fn http_failure_is_infrastructure_failure() {
        let outcome = run_with(
            |_m: &[Message], _t: &[ToolSpec], _to: u64| Err(ModelError::Http("refused".into())),
            FaultMode::Reliable,
            &limits(),
        );
    ...
    fn malformed_response_is_infrastructure_failure() {
        let outcome = run_with(
            |_m: &[Message], _t: &[ToolSpec], _to: u64| Err(ModelError::Parse("no choices".into())),
            FaultMode::Reliable,
            &limits(),
        );
        assert_eq!(
    ...
    #[test]
    fn core_projects_outcome_and_oracle_is_separate() {
        let outcome = run_with(scripted(&[FINAL]), FaultMode::Reliable, &limits());
        let c = core(outcome);
        assert_eq!(c.termination_reason, "completed");
        assert!(c.agent_failure.is_none());
        assert!(c.infrastructure_failure.is_none());
        assert!(c.final_answer.is_some());
        // The oracle is applied by the runner, never by the kernel.
        let task = crate::oracle::TaskSpec {
            id: "T".into(),
            name: "t".into(),
            family: "direct_set".into(),
            split: crate::oracle::TaskSplit::Promotion,
            prompt: "task".into(),
        ...
        assert!(!eval.success);
        assert!(eval.reasons.iter().any(|r| r.contains("x")));
    }
    ...
    // --- 0015 deadline-semantics kernel tests (network-free) --------

    #[test]
    fn normal_request_records_the_registered_deadline_evidence() {
        let outcome = run_with(scripted(&[FINAL]), FaultMode::Reliable, &limits());
        let q = &outcome.model_requests[0];
        assert_eq!(q.request_started_elapsed_ms, 0);
        assert!(q.requested_timeout_ms > 0);
        // requested timeout = min(remaining, ceiling) and never exceeds
        // the remaining episode budget.
        let remaining = 600_000u64 - q.request_started_elapsed_ms;
        assert!(q.requested_timeout_ms <= remaining);
        assert!(q.requested_timeout_ms <= 600_000);
        assert!(q.request_finished_elapsed_ms >= q.request_started_elapsed_ms);
        assert!(q.request_finished_elapsed_ms <= 600_000);
        assert!(!q.timed_out);
        assert!(q.response_accepted);
    }

    #[test]
    fn deadline_reached_before_start_terminates_without_a_request() {
        // A zero deadline: the start check (`start >= deadline`) trips
        // on the very first loop iteration; no endpoint call may be
        // made and no request record emitted.
        let mut calls = 0u32;
        let outcome = run_with(
            |_m: &[Message], _t: &[ToolSpec], _to: u64| {
                calls += 1;
                Err(ModelError::Http("must never be called".into()))
            },
            FaultMode::Reliable,
            &short_deadline_ms(0),
        );
        ...
        assert_eq!(
            calls, 0,
            "no endpoint request may be made after the deadline"
        );
        assert!(
            outcome.model_requests.is_empty(),
            "no request record for a request never started"
        );
        assert!(outcome.final_answer.is_none());
    }

    #[test]
    fn late_response_is_discarded_and_cannot_influence_the_trajectory() {
        // The scripted model sleeps past the (short) deadline and then
        // returns a final answer. The kernel MUST discard it: no
        // assistant message, no final answer, no subsequent turn, and
        // the episode is a valid deadline infrastructure failure.
        let mut tools = ToolExecutor::fresh(&json!({"x": "EMPTY", "y": "B"}), FaultMode::Reliable);
        let specs = ToolExecutor::specs();
        let outcome = run_episode(
            |_m: &[Message], _t: &[ToolSpec], _to: u64| {
                std::thread::sleep(std::time::Duration::from_millis(40));
                let raw: Value = serde_json::from_str(FINAL).unwrap();
                parse_response(&raw)
                    .map(|r| (r, ChannelBinding::default()))
                    .map_err(ModelError::Parse)
            },
        ...
        assert!(outcome.termination_reason.is_infrastructure());
        // Discarded: no assistant message from the late response.
        assert_eq!(outcome.model_turn_count, 0);
        assert!(outcome.final_answer.is_none());
        assert!(
            outcome
        ...
        assert_eq!(q.request_started_elapsed_ms, 0);
        assert!(
            q.request_finished_elapsed_ms > 10,
            "returned after the deadline"
        );
        assert!(
            !q.response_accepted,
            "a late response must be recorded as not accepted"
        );
        // No subsequent request may exist: exactly one request.
        assert_eq!(outcome.model_requests.len(), 1);
    }

    #[test]
    fn late_tool_call_response_executes_no_tool() {
        let mut tools = ToolExecutor::fresh(&json!({"x": "EMPTY", "y": "B"}), FaultMode::Reliable);
        let specs = ToolExecutor::specs();
        let outcome = run_episode(
            |_m: &[Message], _t: &[ToolSpec], _to: u64| {
                std::thread::sleep(std::time::Duration::from_millis(40));
                let raw: Value = serde_json::from_str(WRITE_X_A).unwrap();
                parse_response(&raw)
                    .map(|r| (r, ChannelBinding::default()))
                    .map_err(ModelError::Parse)
            },
        ...
        // The late tool call must not execute: the state is untouched.
        assert!(outcome.executed_tool_calls.is_empty());
        assert_eq!(tools.final_state_value()["x"], "EMPTY");
        // ...and must not appear in the model-visible transcript.
        assert!(
            outcome
                .conversation
                .iter()
    ...
    #[test]
    fn transport_timeout_is_classified_and_recorded() {
        let outcome = run_with(
            |_m: &[Message], _t: &[ToolSpec], _to: u64| {
                Err(ModelError::Timeout("timed out".into()))
            },
            FaultMode::Reliable,
            &limits(),
        );
