...
//! Experiment 0015 (the frozen end-to-end self-evolution
//! confirmation).
//!
...
/// The registered deadline contract (0015 single source of truth).
///
...
/// The registered audited-execution-channel block (0015, the
...
pub struct Design {
    ...
    /// and the return/teardown tolerance (0015 deadline-safety).
    ...
    /// fallback model. In 0015 the model client is bound to the
    ...
    pub model: String,
    /// The registered audited execution channel (0015). The upstream
...
impl Design {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        // Audited execution channel (0015).
        let c = &self.channel;
    ...
    /// wrapped — the 0015 registered schedule has five conditions, so
...
pub struct RunIdentity {
    /// The registered run identity (design `run_id`), e.g. "0015-r1".
    pub run_id: String,
...
pub struct ModelRequestRecord {
    ...
    // --- 0015 request-level deadline evidence (mandatory on every real
    ...
    pub timed_out: bool,
    // --- 0015 audited-execution-channel evidence (mandatory on every
    ...
    /// runtime (never by the model): `0015-r1-<STAGE>-NNNNNN` with a
...
/// The per-request audited-channel binding (0015): the channel-issued
...
pub struct CostBlock {
    ...
    pub model_wait_time_ms: u64,
    pub tool_execution_time_us: u64,
    // --- 0015 deadline summary metrics (re-derived from the
...
/// The 0015 registered request-level deadline contract (spec §28).
///
...
fn verify_record_core<R: RecordCoreLike>(
    ...
    // 0015: the request-level deadline contract is the authoritative
...
pub fn build_synthetic_artifacts() -> SyntheticArtifacts {
    ...
    let design = Design::load(&dir.join("design.json")).expect("0015 design manifest loads");
