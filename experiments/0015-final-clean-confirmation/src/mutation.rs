//! Bounded prompt-mutation generation (Experiment 0015-specific).
//!
...
//!
//! Anti-seeding (0015 critical requirement): no candidate from
//! Experiments 0010 or 0011, no candidate from the 0012 run (C1..C4,
//! including its independently rediscovered read-after-write
//! policies), and no Experiment 0009 repair text is copied, hard-coded,
//! hinted, or seeded anywhere in this crate — not in the mutator
//! prompt, not in the runtime code, not in the retry message. The
//! frozen mutator prompt defines the OUTPUT SCHEMA only and carries
//! zero behavioral hints; if the fresh mutator independently
//! rediscoveries a semantically similar policy, that is ACCEPTED as
//! independent rediscovery — never rejected post hoc. The similarity
//! question is asked only AFTER the formal result is frozen (post-hoc
//! diagnostic, never part of the verdict).
//!
//! Explicit output schema (carried over from 0012 — the fix of the
//! 0011 failure): the 0011-r1 failure was a *structural*
//! schema-adherence failure (the model returned the wrong key / a list
//! of bare strings because the prompt said "return JSON in the required
//! schema" without stating the schema). The frozen mutator prompt
//! (a) states the exact structure, (b) the validator enforces that
//! exact structure with SHAPE-INFORMATIVE errors
//! ([EXPECTED_STRUCTURE]), and (c) on the single registered retry
//! (max 2 attempts) feeds back the machine errors plus the expected
...
/// Maximum number of full mutation-generation attempts (0015: the
...
/// Terms a candidate suffix must NOT contain (0015 spec §16). Compared
/// case-insensitively against the lower-cased suffix. "state_read"/
/// "state_write" ARE allowed (visible tools). Separately, a suffix
/// must not contain any registered task ID or any `V15_*` literal.
...
/// Machine-check for the 0015 synthetic state literal pattern
/// `V15_[A-Z0-9_]+`. Returns true when any occurrence exists.
pub fn contains_v14_literal(s: &str) -> bool {
    let needle = b"V15_";
...
pub fn suffix_content_errors(suffix: &str, registry: &[TaskSpec]) -> Vec<String> {
    ...
    if contains_v14_literal(suffix) {
        errors.push("suffix contains a synthetic V15_* state literal".to_string());
    }
...
/// The shape-informative retry message for an invalid attempt (0015
...
pub fn validate_mutator_response(
    ...
    // --- structural validation (the 0015 explicit schema) -------------
    let mut errors = Vec::new();
...
pub struct GenerationAttempt {
    ...
    /// 0015 audited channel: the channel-issued request identity and
    /// the SHA-256 of the exact request body bytes that traversed the
    /// registered gateway for this attempt (empty only in
    /// network-free/synthetic records).
...
mod tests {
    ...
    fn design() -> Design {
        crate::experiment::load_design().expect("0015 design manifest loads and validates")
    }
    ...
    #[test]
    fn v14_literal_detection() {
        assert!(contains_v14_literal("use V15_DA_0 here"));
        assert!(contains_v14_literal("V15_X"));
        assert!(!contains_v14_literal("no literals"));
        assert!(!contains_v14_literal("the word V15_ alone"));
        assert!(!contains_v14_literal(
            "old V_D1_0 literal is not a 0015 literal"
        ));
    }
    ...
    fn forbidden_terms_and_task_ids_rejected() {
        ...
        for bad in [
            ...
            "Set V15_PA_1 whenever the state appears to need a change now.",
        ] {
    ...
    fn frozen_prompt_documents_the_explicit_schema() {
        // The 0015 fix: the frozen mutator prompt must state the exact
    ...
    fn mutation_input_whitelist_and_leak_check() {
        ...
        let sel = reg.iter().find(|t| t.id == "S15A").unwrap();
