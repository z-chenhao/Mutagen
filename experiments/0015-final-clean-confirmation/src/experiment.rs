//! Experiment 0015 — Frozen End-to-End Self-Evolution Confirmation:
...
//!      (0015 mechanism, `src/stage_freeze.rs`):
//!      B needs only (1); C needs (1)+(B freeze); D needs (1)+(C
//!      freeze); E needs (1)+(D freeze) and a non-G0 selected
//!      candidate.
//!
//! 0015 audited-execution-channel: EVERY model request of every live
//! stage traverses the registered channel gateway (`src/channel.rs`):
//! the runtime allocates the request identity, binds the exact body
//! bytes with the channel capability, and the gateway validates,
//! ledger-appends (authorized or not), and forwards. Each stage, after
//! its episodes, re-verifies the channel (hash chain + zero
//! unauthorized + one-to-one reconciliation) and freezes a channel
//! checkpoint with the stage; the next stage re-verifies the frozen
//! prefix before its first request. After Stage E the channel is
//! finalized (sealed ledger, gateway refuses all further requests).
//!
//! If ANY check fails, NO model request is made and the run is
//! INCONCLUSIVE (no patching, no continuation, no restart).
//!
//! No-restart rule (0015 critical): a live stage whose output already
...
use crate::protocol::{ChatRequest, Message, sha256_bytes};
use crate::stage_freeze;
use crate::tools::{FaultMode, ToolExecutor};
use crate::trace::ChannelBinding;
use crate::trace::{
...
const RAW_DISCOVERY_TRAJ: &str = "0015-discovery-trajectories.jsonl";
const RAW_DISCOVERY_SUMMARY: &str = "0015-discovery-summary.json";
const RAW_MUTATION_GENERATION: &str = "0015-mutation-generation.json";
const RAW_SELECTION_TRAJ: &str = "0015-selection-trajectories.jsonl";
const RAW_SELECTION_SUMMARY: &str = "0015-selection-summary.json";
const RAW_PROMOTION_TRAJ: &str = "0015-promotion-trajectories.jsonl";
const RAW_PROMOTION_SUMMARY: &str = "0015-promotion-summary.json";

/// The channel ledger file (runtime evidence; frozen per stage by the
/// stage-freeze mechanism for ledger-prefix continuity).
const CHANNEL_LEDGER_FILE: &str = channel::CHANNEL_LEDGER;
...
pub fn load_tasks(path: &Path) -> Result<TaskFile, String> {
    ...
    if file.experiment_id != "0015" {
        return Err(format!(
            "task registry experiment_id {:?} does not match this experiment (0015)",
            file.experiment_id
        ));
    }
    file.validate()
...
pub fn frozen_run(manifest_path: &Path) -> Result<(RunIdentity, freeze::RunManifest), String> {
    ...
    // 0015: the committed channel manifest must still byte-match the
    // run-manifest binding (it is a runtime artifact, not a frozen
    // design file, so it is checked HERE, against the manifest hash).
    let cm_bytes = std::fs::read(crate_dir().join(channel::CHANNEL_MANIFEST))
        .map_err(|e| format!("CHANNEL VIOLATION — channel manifest unreadable: {e}"))?;
    if sha256_bytes(&cm_bytes) != manifest.channel_manifest_sha256 {
        return Err(
            "CHANNEL VIOLATION — the channel manifest bytes differ from the run-manifest binding (tamper or reinitialization); the run identity is permanently invalid".to_string(),
        );
    }
    Ok((run, manifest))
}
...
/// (0015 mechanism #2). Any violation ⇒ the next live stage refuses to
...
/// No-restart rule (0015 critical): a live stage whose output file
...
fn execute_episode(
    ...
    task: &TaskSpec,
    full_prompt: &str,
    channel: &RefCell<ChannelSession>,
    stage: &str,
    ...
    let outcome = kernel::run_episode(
        |messages, tool_specs, requested_timeout_ms| {
            // 0015 audited channel: the runtime builds the request body,
            // has the channel session allocate the request identity and
            // mint the authorization bound to the exact body bytes,
            // and sends through the registered gateway. The gateway hop
            // is part of the request elapsed time (no deadline reset).
            let request = ChatRequest::agent(
                client.model(),
                messages.to_vec(),
                tool_specs.to_vec(),
                temperature,
            );
            let body = request.to_body().map_err(crate::model::ModelError::Parse)?;
            let auth = channel.borrow_mut().authorize(stage, &body);
            client
                .send(&body, Duration::from_millis(requested_timeout_ms), &auth)
                .map(|r| {
                    (
                        r,
                        ChannelBinding {
...
/// 0015 deliberately has no `--family` / `--task` live filter: any
...
pub fn cmd_discover(args: DiscoverArgs) -> Result<(), String> {
    ...
    // 1b. 0015 channel gate: the audited channel must be up, armed
    // (after the A1 arm), single-process, and identity-bound. Then DO
    // NOT CHECK THE MODEL ENDPOINT — start the stage directly.
    channel::stage_channel_gate(&manifest, &run)?;
    let channel = RefCell::new(channel::ChannelSession::open(&crate_dir())?);
    ...
    // 0015 channel audit for Stage B: chain + zero unauthorized +
    // one-to-one reconciliation of every B request against the ledger.
    let channel_v = channel::channel_violations(
        &crate_dir(),
        &channel::all_recorded(&records, &None, &[], &[]),
    )
    .map_err(|e| format!("channel verification failed: {e}"))?;
    report_channel("B", &channel_v);
    ...
    if !channel_v.is_empty() {
        return Err(
            "channel verification violations — INCONCLUSIVE: no stage-B freeze may be written; stop"
                .into(),
        );
    }

    // Channel checkpoint B (frozen with the stage).
    let entries = read_ledger_rows()?;
    let recorded_b = channel::all_recorded(&records, &None, &[], &[]);
    channel::build_checkpoint(&crate_dir(), &design.run_id, "B", &entries, &recorded_b)?;

    // Gates pass + verifier clean + channel clean → the stage is
    // FROZEN: 5 artifacts (3 evidence + channel checkpoint + ledger).
    ...
    let b_freeze = stage_freeze::build_stage_freeze(
        ...
        &[
            ...
            (
                crate_rel_path(channel::CHECKPOINT_B),
                runtime_path(channel::CHECKPOINT_B),
            ),
            (
                crate_rel_path(CHANNEL_LEDGER_FILE),
                runtime_path(CHANNEL_LEDGER_FILE),
            ),
        ],
    )?;
    ...
    println!(
        "NEXT — commit B artifacts + manifest together (channel ledger + checkpoint included), then run Stage C:\n  git add {t} {s} {mi} {ck} {lg} {mf}\n  git commit -m \"exp: freeze 0015 discovery evidence\"\n  cargo run -p mutagen-exp-0015 -- generate",
        t = raw_rel_path(RAW_DISCOVERY_TRAJ),
        s = raw_rel_path(RAW_DISCOVERY_SUMMARY),
        mi = crate_rel_path(MUTATION_INPUT),
        ck = crate_rel_path(channel::CHECKPOINT_B),
        lg = crate_rel_path(CHANNEL_LEDGER_FILE),
        mf = crate_rel_path(STAGE_B_FREEZE),
    );
    Ok(())
}

/// Read the current channel ledger rows.
fn read_ledger_rows() -> Result<Vec<channel_ledger::ChannelEntry>, String> {
    let dir = crate_dir();
    let manifest = channel::ChannelManifest::load(&dir.join(channel::CHANNEL_MANIFEST))
        .map_err(|e| format!("channel manifest: {e}"))?;
    let ledger = channel_ledger::Ledger::open(
        &dir.join(channel::CHANNEL_LEDGER),
        &manifest.ledger_genesis_hash,
    )?;
    ledger.entries()
}

/// Report the channel audit for one stage (0015).
fn report_channel(stage: &str, violations: &[String]) {
    if violations.is_empty() {
        println!(
            "[channel/{stage}] channel verifier: OK (chain intact, zero unauthorized, fully reconciled)"
        );
    } else {
        println!(
            "[channel/{stage}] channel verifier: {} violation(s):",
            violations.len()
        );
        for v in violations {
            println!("  - {v}");
...
pub fn cmd_generate(args: GenerateArgs) -> Result<(), String> {
    ...
    // 1b. 0015: the channel (armed, single-process, identity-bound) +
    // ledger prefix continuity with the frozen B checkpoint.
    channel::stage_channel_gate(&manifest, &run)?;
    let ck = channel::load_checkpoint("B")?;
    let prefix_v = channel::checkpoint_prefix_violations(&crate_dir(), &ck)?;
    if !prefix_v.is_empty() {
        return Err(format!(
            "CHANNEL VIOLATION — ledger prefix continuity with the frozen B checkpoint:\n  - {}",
            prefix_v.join("\n  - ")
        ));
    }
    let channel = RefCell::new(channel::ChannelSession::open(&crate_dir())?);
    ...
    for attempt_no in 1..=mutation::MAX_GENERATION_ATTEMPTS {
        let t0 = Instant::now();
        // 0015 audited channel: the mutation-generation request
        // traverses the registered gateway with stage=C. The single
        // registered timeout is the design-registered request-timeout
        // ceiling (Stage C is not an agent episode; the per-episode
        // deadline contract applies to Stages B/D/E agent requests).
        let request =
            ChatRequest::mutator(client.model(), messages.clone(), design.mutator_temperature);
        let body = request
            .to_body()
            .map_err(|e| format!("mutation request serialization: {e}"))?;
        let auth = channel.borrow_mut().authorize("C", &body);
        let channel_request_id = auth.request_id.clone();
        let request_body_sha = auth.body_sha256.clone();
        ...
        attempts.push(GenerationAttempt {
            ...
            wall_time_ms: wall,
            channel_request_id: channel_request_id.clone(),
            request_body_sha256: request_body_sha.clone(),
        });
    ...
    // 0015 channel audit for Stage C: B + C requests reconciled.
    let discovery_records =
        read_jsonl::<DiscoveryRecord>(&raw_path(RAW_DISCOVERY_TRAJ)).unwrap_or_default();
    // The artifact (already constructed, with its attempts) is the
    // audit's authority for the Stage C recorded requests.
    let generation_artifact = Some(generation.clone());
    let channel_v = channel::channel_violations(
        &crate_dir(),
        &channel::all_recorded(&discovery_records, &generation_artifact, &[], &[]),
    )
    .map_err(|e| format!("channel verification failed: {e}"))?;
    report_channel("C", &channel_v);
    ...
    if !channel_v.is_empty() {
        return Err(
            "channel verification violations — INCONCLUSIVE: no stage-C freeze may be written; stop"
                .into(),
        );
    }

    // Channel checkpoint C (frozen with the stage).
    let entries = read_ledger_rows()?;
    let recorded_c = channel::all_recorded(&discovery_records, &generation_artifact, &[], &[]);
    channel::build_checkpoint(&crate_dir(), &design.run_id, "C", &entries, &recorded_c)?;

    // Complete + clean + channel clean → the stage is FROZEN.
    let c_freeze = stage_freeze::build_stage_freeze(
        ...
        &[
            ...
            (
                crate_rel_path(channel::CHECKPOINT_C),
                runtime_path(channel::CHECKPOINT_C),
            ),
            (
                crate_rel_path(CHANNEL_LEDGER_FILE),
                runtime_path(CHANNEL_LEDGER_FILE),
    ...
    println!(
        "NEXT — commit C artifacts + manifest together (channel ledger + checkpoint included), then run Stage D:\n  git add {} {} {ck} {lg} {mf}\n  git commit -m \"exp: freeze 0015 generated candidate pool\"\n  cargo run -p mutagen-exp-0015 -- select",
        raw_rel_path(RAW_MUTATION_GENERATION),
        crate_rel_path(CANDIDATE_POOL),
        ck = crate_rel_path(channel::CHECKPOINT_C),
        lg = crate_rel_path(CHANNEL_LEDGER_FILE),
        mf = crate_rel_path(STAGE_C_FREEZE),
    );
    Ok(())
}
...
pub fn cmd_select(args: SelectArgs) -> Result<(), String> {
    ...
    // 1b. 0015: channel gate + ledger prefix continuity with the
    // frozen C checkpoint.
    channel::stage_channel_gate(&manifest, &run)?;
    let ck = channel::load_checkpoint("C")?;
    let prefix_v = channel::checkpoint_prefix_violations(&crate_dir(), &ck)?;
    if !prefix_v.is_empty() {
        return Err(format!(
            "CHANNEL VIOLATION — ledger prefix continuity with the frozen C checkpoint:\n  - {}",
            prefix_v.join("\n  - ")
        ));
    }
    let channel = RefCell::new(channel::ChannelSession::open(&crate_dir())?);
    ...
    // 0015 channel audit for Stage D: B + C + D requests reconciled.
    let discovery_records =
        read_jsonl::<DiscoveryRecord>(&raw_path(RAW_DISCOVERY_TRAJ)).unwrap_or_default();
    ...
    let channel_v = channel::channel_violations(
        &crate_dir(),
        &channel::all_recorded(&discovery_records, &generation_artifact, &records, &[]),
    )
    .map_err(|e| format!("channel verification failed: {e}"))?;
    report_channel("D", &channel_v);
    ...
    if !channel_v.is_empty() {
        return Err(
            "channel verification violations — INCONCLUSIVE: no stage-D freeze may be written; stop"
                .into(),
        );
    }

    // Channel checkpoint D (frozen with the stage).
    let entries = read_ledger_rows()?;
    let recorded_d = channel::all_recorded(&discovery_records, &generation_artifact, &records, &[]);
    channel::build_checkpoint(&crate_dir(), &design.run_id, "D", &entries, &recorded_d)?;

    // The stage completed and is clean (selection + deadline + channel
    // verifiers all clean) → the stage is FROZEN.
    let d_freeze = stage_freeze::build_stage_freeze(
        ...
        &[
            ...
            (
                crate_rel_path(channel::CHECKPOINT_D),
                runtime_path(channel::CHECKPOINT_D),
            ),
            (
                crate_rel_path(CHANNEL_LEDGER_FILE),
                runtime_path(CHANNEL_LEDGER_FILE),
    ...
    println!(
        "NEXT — commit D artifacts + manifest together (channel ledger + checkpoint included):\n  git add {} {} {} {ck} {lg} {mf}\n  git commit -m \"exp: freeze 0015 selected candidate before promotion\"",
        raw_rel_path(RAW_SELECTION_TRAJ),
        raw_rel_path(RAW_SELECTION_SUMMARY),
        crate_rel_path(SELECTED_CANDIDATE),
        ck = crate_rel_path(channel::CHECKPOINT_D),
        lg = crate_rel_path(CHANNEL_LEDGER_FILE),
        mf = crate_rel_path(STAGE_D_FREEZE),
    );
...
pub fn cmd_promote(args: PromoteArgs) -> Result<(), String> {
    ...
    // frozen D checkpoint.
    channel::stage_channel_gate(&manifest, &run)?;
    let ck = channel::load_checkpoint("D")?;
    let prefix_v = channel::checkpoint_prefix_violations(&crate_dir(), &ck)?;
    if !prefix_v.is_empty() {
        return Err(format!(
            "CHANNEL VIOLATION — ledger prefix continuity with the frozen D checkpoint:\n  - {}",
            prefix_v.join("\n  - ")
        ));
    }
    let channel = RefCell::new(channel::ChannelSession::open(&crate_dir())?);
    ...
    // 0015: channel FINALIZE (after Stage E): stop accepting new
    // requests, flush the ledger, seal the final head, write
    // channel-final.json (the gateway then refuses all further
    // requests). Local control socket only — zero HTTP.
    channel::cmd_channel_finalize()?;

    // 0015 final channel audit: B + C + D + E reconciled against the
    ...
    let mut final_doc = serde_json::from_slice::<channel::ChannelFinal>(
        &std::fs::read(runtime_path(channel::CHANNEL_FINAL))
            .map_err(|e| format!("channel-final.json missing: {e}"))?,
    )
    .map_err(|e| format!("channel-final.json invalid: {e}"))?;

    // Complete the sealed document with the final reconciliation
    // counts (recomputed from the artifacts + the sealed ledger).
    let genesis = channel::ChannelManifest::load(&runtime_path(channel::CHANNEL_MANIFEST))
        .map(|m| m.ledger_genesis_hash)
        .map_err(|e| e.to_string())?;
    let chain_violations = channel_ledger::verify_chain(&entries, &genesis);
    let (matched, unmatched) = channel::match_counts_public(&entries, &all_rec);
    let missing = channel::missing_recorded(&entries, &all_rec);
    final_doc.matched_count = matched;
    final_doc.unmatched_count = unmatched;
    final_doc.missing_recorded_count = missing;
    final_doc.duplicate_request_ids = channel::duplicate_ids(&entries);
    final_doc.hash_chain_violations = chain_violations.len() as u64;
    let final_text =
        serde_json::to_string_pretty(&final_doc).map_err(|e| format!("channel-final: {e}"))?;
    std::fs::write(
        runtime_path(channel::CHANNEL_FINAL),
        format!("{final_text}\n"),
    )
    .map_err(|e| format!("cannot write channel-final.json: {e}"))?;

    // Seal integrity: the sealed ledger must re-verify and the seal
    // must match the ledger (the formal request-count rule, 0015:
    // gateway forwarded count == experiment-recorded live request
    // total; artifacts are the authority, no hardcoded count).
    let mut seal_v = Vec::new();
    if !chain_violations.is_empty() {
        seal_v.push(format!(
            "{} ledger hash-chain violation(s) after finalization",
            chain_violations.len()
        ));
    }
    if final_doc.final_entry_count != entries.len() as u64 {
        seal_v.push(format!(
            "channel-final final_entry_count {} != ledger rows {}",
            final_doc.final_entry_count,
            entries.len()
        ));
    }
    let counts = channel_ledger::count_entries(&entries);
    if final_doc.unauthorized_count != counts.unauthorized {
        seal_v.push("channel-final unauthorized_count disagrees with the ledger".to_string());
    }
    if final_doc.forwarded_count != counts.authorized_forwarded {
        seal_v.push("channel-final forwarded_count disagrees with the ledger".to_string());
    }
    if final_doc.gateway_restarts != 0 {
        seal_v.push(format!(
            "{} gateway restart(s) detected after A1 — INCONCLUSIVE",
            final_doc.gateway_restarts
        ));
    }
    let recorded_total = all_rec.iter().filter(|r| !r.1.is_empty()).count() as u64;
    if counts.authorized_forwarded != recorded_total {
        seal_v.push(format!(
            "gateway forwarded count {} != experiment-recorded live request count {}",
            counts.authorized_forwarded, recorded_total
        ));
    }
    if !seal_v.is_empty() {
        return Err(format!(
    ...
    if !channel_v.is_empty() {
        return Err(
            "final channel verification violations — the run is INCONCLUSIVE (record the result; do not rerun the same run identity)".into(),
        );
    }

    // The channel integrity summary (0015 required final report).
    println!(
        "[channel-final] session={} ledger_entries={} forwarded={} unauthorized={} matched={} unmatched={} missing={} duplicates={} hash_chain_violations={} gateway_restarts={}",
        final_doc.session_id,
        final_doc.final_entry_count,
        final_doc.forwarded_count,
        final_doc.unauthorized_count,
        final_doc.matched_count,
        final_doc.unmatched_count,
        final_doc.missing_recorded_count,
        final_doc.duplicate_request_ids,
        final_doc.hash_chain_violations,
        final_doc.gateway_restarts,
    );
    println!(
        "[channel-final] stage counts: B={} C={} D={} E={} | experiment total B+C+D+E = {} requests, gateway forwarded = {}",
        final_doc.stage_b_count,
        final_doc.stage_c_count,
        final_doc.stage_d_count,
        final_doc.stage_e_count,
        recorded_total,
        final_doc.forwarded_count,
    );
    Ok(())
...
pub fn cmd_init_run(args: InitRunArgs) -> Result<(), String> {
    ...
    let manifest = freeze::build_manifest(&root, &commit, "0015-r1", "0015")?;
    ...
    println!(
        "  channel: session {} | protocol {} | gateway {} | upstream sha256 {}",
        manifest.channel_session_id,
        manifest.channel_protocol_version,
        manifest.channel_gateway_endpoint,
        &manifest.channel_upstream_endpoint_sha256
            [..12.min(manifest.channel_upstream_endpoint_sha256.len())],
    );
    println!(
        "AFTER THIS COMMIT (A1): the audited channel is the ONLY route to the model.\n  git add {} {}\n  git commit -m \"exp: freeze 0015 run and channel identity\"\n  cargo run -p mutagen-exp-0015 -- channel-arm\n  cargo run -p mutagen-exp-0015 -- discover --run-manifest {m}",
        args.manifest.display(),
        crate_dir().join(crate::channel::CHANNEL_MANIFEST).display(),
        m = args.manifest.display(),
    );
    Ok(())
}
...
pub fn print_a0_commands(commit_message: &str) {
    ...
    println!(
        "  git rev-parse HEAD   # → pass to: cargo run -p mutagen-exp-0015 -- init-run --commit <SHA>"
    );
    println!(
        "  THEN (after A0, before A1): cargo run -p mutagen-exp-0015 -- channel-init   # session + UNARMED gateway (zero model contact)"
    );
    println!(
        "  THEN: init-run (A1) → git add run-manifest.json channel-manifest.json → git commit → channel-arm"
    );
    println!(
        "  AFTER ARM: the channel is the ONLY route to the model. No curl, no /v1/models, no probe, no smoke request — any other gateway access is logged and makes the run INCONCLUSIVE."
    );
}
...
pub fn source_freeze_checks() -> Vec<Check> {
    ...
    fn mk(files: &[(&str, &str)], commit: &str) -> freeze::RunManifest {
        ...
        freeze::RunManifest {
            ...
            channel_protocol_version: "mutagen-channel/v1".into(),
            channel_gateway_endpoint: "http://127.0.0.1:18150/v1".into(),
            channel_upstream_endpoint_sha256: String::new(),
            channel_session_id: String::new(),
            channel_manifest_sha256: String::new(),
        }
    }
...
/// `tool_choice = "auto"` (one tool call per turn, frozen in 0015).
...
/// The 0015 deadline-contract checks (spec §59): the registered
    ...
    let late_check = {
        ...
        let outcome = crate::kernel::run_episode(
            |_m: &[Message], _t: &[crate::protocol::ToolSpec], _to: u64| {
                ...
                .unwrap();
                let binding = crate::trace::ChannelBinding::default();
                Ok((crate::protocol::parse_response(&raw).unwrap(), binding))
            },
            &mut tools,
            &specs,
            "sys",
...
pub fn cmd_preflight(manifest: Option<&Path>) -> Result<(), String> {
    println!("== Experiment 0015 preflight (network-free) ==");
    let mut fail = false;
    ...
    // 5b. Deadline contract (carried over from 0013 unchanged).
    for c in deadline_contract_checks() {
        report(c.name, c.passed, &c.detail);
    }
    // 5c. Audited execution channel (the 0015 semantic delta):
    // static bypass guard + the behavioral suite (in-process gateway
    // + fake in-process upstream; no real model contact) + the
    // synthetic D1 final-reconciliation pair (B + C + D + E exact;
    // D-omitted failure — the 0014-r1 defect shape).
    for c in channel::bypass_guard_checks() {
        report(c.name, c.passed, &c.detail);
    }
    for c in channel::channel_preflight_checks() {
        report(c.name, c.passed, &c.detail);
    }
    for c in channel::synthetic_final_reconciliation_checks() {
        report(c.name, c.passed, &c.detail);
    }
...
pub fn self_test() -> Result<(), String> {
    ...
    // --- 5b. Audited execution channel (0015 semantic delta) ------------
    println!("-- 5b. audited execution channel (0015) --");
    for c in channel::bypass_guard_checks() {
        self_test_pass(&mut failures, c.name, c.passed, &c.detail);
    }
    for c in channel::channel_preflight_checks() {
            ...
            .and_then(|v| v.as_array_mut())
        {
            if let Some(st) = eps.first_mut().and_then(|e| e.get_mut("final_state")) {
                if let Some(o) = st.as_object_mut() {
                    o.insert("x".into(), Value::String("V15_SA_0".into())); // selection literal
                }
            }
        }
        let v = trace::verify_discovery(
