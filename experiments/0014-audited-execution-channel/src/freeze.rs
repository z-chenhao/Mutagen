//! The mechanical source-freeze system (Experiment 0014, carried
//! forward in mechanism from Experiment 0011).
//!
//! This is one of the two decisive 0014 mechanisms (together with the
//! commit-level stage-artifact freeze in `src/stage_freeze.rs`).
//! Experiment 0010 was declared inconclusive because the *executable
//! source changed after live model execution had begun* (a bugfix was
//! committed mid-selection). 0011 (and now 0014) enforces the freeze
//! mechanically:
//!
...
/// `selected-candidate.json`, `stage-*-freeze.json`, raw trajectories
/// and summaries) are NOT frozen design files — they are run outputs
/// with their own provenance binding (stage freezes).
///
/// Exactly 23 files. The results document is NOT frozen.
pub const FROZEN_PATHS: &[&str] = &[
    "experiments/0014-audited-execution-channel/Cargo.toml",
    "experiments/0014-audited-execution-channel/Cargo.lock",
    "experiments/0014-audited-execution-channel/design.json",
    "experiments/0014-audited-execution-channel/tasks.json",
    "experiments/0014-audited-execution-channel/stress-profile.json",
    "experiments/0014-audited-execution-channel/prompts/incumbent.md",
    "experiments/0014-audited-execution-channel/prompts/mutator.md",
    "experiments/0014-audited-execution-channel/src/main.rs",
    "experiments/0014-audited-execution-channel/src/channel.rs",
    "experiments/0014-audited-execution-channel/src/channel_ledger.rs",
    "experiments/0014-audited-execution-channel/src/protocol.rs",
    "experiments/0014-audited-execution-channel/src/model.rs",
    "experiments/0014-audited-execution-channel/src/kernel.rs",
    "experiments/0014-audited-execution-channel/src/tools.rs",
    "experiments/0014-audited-execution-channel/src/oracle.rs",
    "experiments/0014-audited-execution-channel/src/mutation.rs",
    "experiments/0014-audited-execution-channel/src/selection.rs",
    "experiments/0014-audited-execution-channel/src/stats.rs",
    "experiments/0014-audited-execution-channel/src/freeze.rs",
    "experiments/0014-audited-execution-channel/src/stage_freeze.rs",
    "experiments/0014-audited-execution-channel/src/trace.rs",
    "experiments/0014-audited-execution-channel/src/experiment.rs",
    "docs/experiments/0014-audited-execution-channel.md",
];
...
/// hashes of every frozen design file AND the audited-execution-channel
/// identity (0014). Once committed, it is the authority every live
/// stage verifies against.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RunManifest {
    ...
    /// The channel session bound into the run (channel-manifest.json,
    /// committed together with this manifest at A1).
    pub channel_session_id: String,
    pub channel_protocol_version: String,
    /// The registered experiment endpoint (the channel gateway).
    pub channel_gateway_endpoint: String,
    /// SHA-256 of the physical upstream endpoint string (the run
    /// manifest binds the HASH; the URL itself lives in the channel
    /// manifest, which is a channel artifact).
    pub channel_upstream_endpoint_sha256: String,
    /// SHA-256 of the exact `channel-manifest.json` bytes at A1.
...
pub fn build_manifest(
    ...
    // The channel manifest MUST exist (channel-init, before A1) and be
    // consistent with the registered design channel block.
    let cm =
        crate::channel::ChannelManifest::load(&crate_dir().join(crate::channel::CHANNEL_MANIFEST))
            .map_err(|e| {
                format!("channel manifest must exist before A1 (run `channel-init` first): {e}")
            })?;
    if cm.run_id != run_id {
        return Err(format!(
            "channel manifest run_id {cm_run_id:?} != run manifest run_id {run_id:?}",
            cm_run_id = cm.run_id
        ));
    }
    if cm.gateway_endpoint != design.channel.gateway_endpoint
        || cm.protocol_version != design.channel.protocol_version
    {
        return Err("channel manifest does not match the registered design channel block".into());
    }
    ...
    let manifest_bytes = std::fs::read(crate_dir().join(crate::channel::CHANNEL_MANIFEST))
        .map_err(|e| format!("cannot read channel manifest: {e}"))?;
    Ok(RunManifest {
        experiment_id: experiment_id.to_string(),
        run_id: run_id.to_string(),
        code_under_test_commit: code_commit.to_string(),
        channel_session_id: cm.session_id.clone(),
        channel_protocol_version: cm.protocol_version.clone(),
        channel_gateway_endpoint: cm.gateway_endpoint.clone(),
        channel_upstream_endpoint_sha256: cm.upstream_endpoint_sha256.clone(),
        channel_manifest_sha256: sha256_bytes(&manifest_bytes),
...
pub fn crate_dir() -> PathBuf {
    std::env::var("CARGO_MANIFEST_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("experiments/0014-audited-execution-channel"))
}
...
mod tests {
    ...
    fn manifest_for(files: &[(&str, &str)], commit: &str) -> RunManifest {
        ...
        RunManifest {
            ...
            channel_protocol_version: String::new(),
            channel_gateway_endpoint: String::new(),
