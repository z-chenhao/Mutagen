//! Bounded prompt-mutation generation (Experiment 0014-specific).
//!
...
//!
//! Anti-seeding (0014 critical requirement): no candidate from
...
/// Maximum number of full mutation-generation attempts (0014: the
...
/// Terms a candidate suffix must NOT contain (0014 spec §16). Compared
/// case-insensitively against the lower-cased suffix. "state_read"/
/// "state_write" ARE allowed (visible tools). Separately, a suffix
/// must not contain any registered task ID or any `V14_*` literal.
...
/// Machine-check for the 0014 synthetic state literal pattern
/// `V14_[A-Z0-9_]+`. Returns true when any occurrence exists.
...
pub fn suffix_content_errors(suffix: &str, registry: &[TaskSpec]) -> Vec<String> {
    ...
    if contains_v14_literal(suffix) {
        errors.push("suffix contains a synthetic V14_* state literal".to_string());
    }
...
/// The shape-informative retry message for an invalid attempt (0014
...
pub fn validate_mutator_response(
    ...
    // --- structural validation (the 0014 explicit schema) -------------
    let mut errors = Vec::new();
...
pub struct GenerationAttempt {
    ...
    /// 0014 audited channel: the channel-issued request identity and
...
mod tests {
    ...
    fn design() -> Design {
        crate::experiment::load_design().expect("0014 design manifest loads and validates")
    }
    ...
    fn v14_literal_detection() {
        assert!(contains_v14_literal("use V14_DA_0 here"));
        assert!(contains_v14_literal("V14_X"));
        assert!(!contains_v14_literal("no literals"));
        assert!(!contains_v14_literal("the word V14_ alone"));
        assert!(!contains_v14_literal(
            "old V_D1_0 literal is not a 0014 literal"
        ));
    }
    ...
    fn forbidden_terms_and_task_ids_rejected() {
        ...
        for bad in [
            ...
            "Set V14_PA_1 whenever the state appears to need a change now.",
        ] {
    ...
    fn frozen_prompt_documents_the_explicit_schema() {
        // The 0014 fix: the frozen mutator prompt must state the exact
    ...
    fn mutation_input_whitelist_and_leak_check() {
        ...
        let sel = reg.iter().find(|t| t.id == "S14A").unwrap();
