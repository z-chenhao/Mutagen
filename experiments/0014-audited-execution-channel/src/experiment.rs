//! Experiment 0014 — Frozen End-to-End Self-Evolution Confirmation:
...
//!      (0014 mechanism, `src/stage_freeze.rs`):
//!      B needs only (1); C needs (1)+(B freeze); D needs (1)+(C
//!      freeze); E needs (1)+(D freeze) and a non-G0 selected
//!      candidate.
//!
//! 0014 audited-execution-channel: EVERY model request of every live
...
//!
//! No-restart rule (0014 critical): a live stage whose output already
...
const RAW_DISCOVERY_TRAJ: &str = "0014-discovery-trajectories.jsonl";
const RAW_DISCOVERY_SUMMARY: &str = "0014-discovery-summary.json";
const RAW_MUTATION_GENERATION: &str = "0014-mutation-generation.json";
const RAW_SELECTION_TRAJ: &str = "0014-selection-trajectories.jsonl";
const RAW_SELECTION_SUMMARY: &str = "0014-selection-summary.json";
const RAW_PROMOTION_TRAJ: &str = "0014-promotion-trajectories.jsonl";
const RAW_PROMOTION_SUMMARY: &str = "0014-promotion-summary.json";
...
pub fn load_tasks(path: &Path) -> Result<TaskFile, String> {
    ...
    if file.experiment_id != "0014" {
        return Err(format!(
            "task registry experiment_id {:?} does not match this experiment (0014)",
            file.experiment_id
        ));
    }
    file.validate()
...
pub fn frozen_run(manifest_path: &Path) -> Result<(RunIdentity, freeze::RunManifest), String> {
    ...
    // 0014: the committed channel manifest must still byte-match the
...
/// (0014 mechanism #2). Any violation ⇒ the next live stage refuses to
...
/// No-restart rule (0014 critical): a live stage whose output file
...
fn execute_episode(
    ...
    let outcome = kernel::run_episode(
        |messages, tool_specs, requested_timeout_ms| {
            // 0014 audited channel: the runtime builds the request body,
...
/// 0014 deliberately has no `--family` / `--task` live filter: any
...
pub fn cmd_discover(args: DiscoverArgs) -> Result<(), String> {
    ...
    // 1b. 0014 channel gate: the audited channel must be up, armed
    ...
    // 0014 channel audit for Stage B: chain + zero unauthorized +
...
/// Report the channel audit for one stage (0014).
...
pub fn cmd_generate(args: GenerateArgs) -> Result<(), String> {
    ...
    // 1b. 0014: the channel (armed, single-process, identity-bound) +
    ...
    for attempt_no in 1..=mutation::MAX_GENERATION_ATTEMPTS {
        let t0 = Instant::now();
        // 0014 audited channel: the mutation-generation request
    ...
    // 0014 channel audit for Stage C: B + C requests reconciled.
...
pub fn cmd_select(args: SelectArgs) -> Result<(), String> {
    ...
    // 1b. 0014: channel gate + ledger prefix continuity with the
    // frozen C checkpoint.
    channel::stage_channel_gate(&manifest, &run)?;
    ...
    // 0014 channel audit for Stage D: B + C + D requests reconciled.
    let discovery_records =
        read_jsonl::<DiscoveryRecord>(&raw_path(RAW_DISCOVERY_TRAJ)).unwrap_or_default();
...
pub fn cmd_promote(args: PromoteArgs) -> Result<(), String> {
    ...
    // 0014: channel FINALIZE (after Stage E): stop accepting new
    ...
    // must match the ledger (the formal request-count rule, 0014:
    ...
    // The channel integrity summary (0014 required final report).
    println!(
...
pub fn cmd_init_run(args: InitRunArgs) -> Result<(), String> {
    ...
    let manifest = freeze::build_manifest(&root, &commit, "0014-r1", "0014")?;
...
pub fn print_a0_commands(commit_message: &str) {
    ...
    println!(
        "  git rev-parse HEAD   # → pass to: cargo run -p mutagen-exp-0014 -- init-run --commit <SHA>"
    );
    println!(
        "  THEN (after A0, before A1): cargo run -p mutagen-exp-0014 -- channel-init   # session + UNARMED gateway (zero model contact)"
    );
    println!(
...
pub fn source_freeze_checks() -> Vec<Check> {
    ...
    fn mk(files: &[(&str, &str)], commit: &str) -> freeze::RunManifest {
        ...
        freeze::RunManifest {
            ...
            channel_gateway_endpoint: "http://127.0.0.1:18140/v1".into(),
...
/// `tool_choice = "auto"` (one tool call per turn, frozen in 0014).
...
/// The 0014 deadline-contract checks (spec §59): the registered
...
pub fn cmd_preflight(manifest: Option<&Path>) -> Result<(), String> {
    println!("== Experiment 0014 preflight (network-free) ==");
    let mut fail = false;
    ...
    // 5c. Audited execution channel (the 0014 semantic delta):
    // static bypass guard + the behavioral suite (in-process gateway
    // + fake in-process upstream; no real model contact).
...
pub fn self_test() -> Result<(), String> {
    ...
    // --- 5b. Audited execution channel (0014 semantic delta) ------------
    println!("-- 5b. audited execution channel (0014) --");
    for c in channel::bypass_guard_checks() {
        self_test_pass(&mut failures, c.name, c.passed, &c.detail);
    }
    for c in channel::channel_preflight_checks() {
            ...
            .and_then(|v| v.as_array_mut())
        {
            if let Some(st) = eps.first_mut().and_then(|e| e.get_mut("final_state")) {
                if let Some(o) = st.as_object_mut() {
                    o.insert("x".into(), Value::String("V14_SA_0".into())); // selection literal
                }
            }
        }
        let v = trace::verify_discovery(
