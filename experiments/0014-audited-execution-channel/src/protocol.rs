//! OpenAI-compatible Chat Completions protocol surface for Experiment 0014.
//!
//! Scope (deliberately minimal, carried over from Experiment 0006):
//! - message roles: system, user, assistant, tool
//! - requests: `model`, `messages`, `tools`, `tool_choice = "auto"`,
//!   `parallel_tool_calls = false` when tools are present; temperatures
//!   are *parameters* (the single source of truth is `design.json`:
//!   `agent_temperature`, `mutator_temperature`)
...
/// tools array). `parallel_tool_calls = false` is sent whenever tools
/// are present (the 0014 frozen agent request semantics, spec §10); the
/// kernel additionally rejects multi-call responses if a provider
/// ignores the flag.
...
impl ChatRequest {
    /// Agent-execution request: the caller-supplied agent temperature
    /// (from `design.json` `agent_temperature`), `tool_choice =
    /// "auto"`, and `parallel_tool_calls = false`. The kernel fails the
    /// episode with `parallel_tool_calls_unsupported` if a provider
    /// nevertheless emits more than one call (spec §14).
    pub fn agent(
        model: &str,
    ...
    /// Mutation-generation request: no tools (no `state_read`, no
    /// `state_write`, no shell, no repository access — spec §15), the
    /// caller-supplied mutation temperature (from `design.json`
    /// `mutator_temperature`).
    pub fn mutator(model: &str, messages: Vec<Message>, temperature: f64) -> Self {
        Self::with_temperature(model, messages, Vec::new(), temperature)
    }
    ...
    /// The exact serialized request body bytes (0014 audited channel:
    /// the experiment runtime serializes the body HERE, hashes it for
    /// the channel authorization, and sends exactly those bytes
    /// through the registered gateway — the body hash is part of the
    /// per-request MAC input, so a silently altered body cannot be
    /// forwarded undetected).
    pub fn to_body(&self) -> Result<String, String> {
        serde_json::to_string(self).map_err(|e| format!("request serialization: {e}"))
    }
}
...
mod tests {
    ...
    // --- frozen request semantics (spec §14/§15) ----------------------

    #[test]
    fn agent_request_serializes_parallel_tool_calls_false_and_tool_choice_auto() {
    ...
    #[test]
    fn mutator_request_carries_no_tools_and_no_tool_choice() {
        let req = ChatRequest::mutator("m", vec![Message::system("s")], 0.7);
        let v = serde_json::to_value(&req).unwrap();
        assert!(
            v.get("tool_choice").is_none(),
            "mutator must omit tool_choice"
        );
        assert!(
            v.get("parallel_tool_calls").is_none(),
            "mutator must omit parallel_tool_calls"
        );
        assert!(
            v["tools"].as_array().unwrap().is_empty(),
            "mutator must carry no tools"
        );
