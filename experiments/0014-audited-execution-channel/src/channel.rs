//! The audited execution channel (Experiment 0014 — the experiment's
...
//! - `capability_key = SHA-256("mutagen-exp-0014-channel" ||
...
//!   (`0014-r1-<stage>-NNNNNN`, stage-local sequence from 1, issued by
...
pub const PROTOCOL_VERSION: &str = "0014-audited-execution-channel-v1";
...
pub fn capability_key(session_id: &str) -> Vec<u8> {
    ...
    hasher.update(b"mutagen-exp-0014-channel");
...
impl ChannelSession {
    ...
    /// (`0014-r1-<stage>-NNNNNN`, stage-local sequence from 1).
...
pub fn channel_preflight_checks() -> Vec<Check> {
    ...
    match crate::experiment::load_design() {
        Ok(design) => {
            ...
            let ok = c.protocol_version == PROTOCOL_VERSION
                && c.gateway_endpoint == "http://127.0.0.1:18140/v1"
    ...
    // 1. Authorized request → forwarded + ledger authorized=true.
    {
        ...
        let auth = hmac_sha256_hex(
            &key,
            &auth_input("0014-test", "B", "0014-test-B-000001", &body_sha),
        );
        let (status, _) = fixture_request(
            ...
            "/v1/chat/completions",
            &[
                ("X-Mutagen-Run", "0014-test"),
                ("X-Mutagen-Stage", "B"),
                ("X-Mutagen-Request-Id", "0014-test-B-000001"),
                ("X-Mutagen-Auth", &auth),
            ],
            &body,
        );
    ...
    // 3. Wrong stage → rejected/logged.
    {
        ...
        let auth = hmac_sha256_hex(
            &key,
            &auth_input("0014-test", "Z", "0014-test-Z-000001", &body_sha),
        );
        let (status, _) = fixture_request(
            &fx.gateway_gateway,
            "POST",
            ...
            &[
                ("X-Mutagen-Run", "0014-test"),
                ("X-Mutagen-Stage", "Z"),
                ("X-Mutagen-Request-Id", "0014-test-Z-000001"),
                ("X-Mutagen-Auth", &auth),
            ],
            &body,
        );
    ...
    // 4. Wrong body hash (tampered body under a valid MAC) → rejected/logged.
    {
        ...
        let auth = hmac_sha256_hex(
            &key,
            &auth_input("0014-test", "B", "0014-test-B-000002", &body_sha),
        );
        ...
        let (status, _) = fixture_request(
            ...
            &[
                ("X-Mutagen-Run", "0014-test"),
                ("X-Mutagen-Stage", "B"),
                ("X-Mutagen-Request-Id", "0014-test-B-000002"),
                ("X-Mutagen-Auth", &auth),
            ],
            &tampered,
        );
