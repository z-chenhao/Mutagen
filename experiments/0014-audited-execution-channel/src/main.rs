//! Experiment 0014 runner: Frozen End-to-End Self-Evolution
...
//!
//! Endpoint discipline (0014 formal protocol condition): these four
...
//! Raw trajectories/summaries: docs/experiments/artifacts/0014-*.
//!
...
pub fn registered_commands() -> Vec<(&'static str, bool)> {
    vec![
        ("self-test", false),
        // 0014 audited-execution-channel commands (all local; the
        // `gateway` subcommand is the channel infrastructure itself,
        // not experiment traffic — the registered live set is exactly
        // the four stage commands).
        ("channel-init", false),
        ("channel-arm", false),
        ("channel-status-local", false),
        ("channel-finalize", false),
        ("gateway", false),
        ("preflight", false),
        ("init-run", false),
...
fn usage() {
    println!(
        "mutagen-exp-0014 — Frozen End-to-End Self-Evolution Confirmation\n\
         \n\
         ...
         binary (the 0011 `run --task` smoke command does not exist in 0014);\n\
    ...
    let result = match command.as_str() {
        ...
        "print-a0" => {
            experiment::print_a0_commands(
                "exp: preregister audited-channel 0014 self-evolution confirmation",
            );
            Ok(())
        }
        ...
        "channel-init" => channel::cmd_channel_init(),
        "channel-arm" => channel::cmd_channel_arm(),
        "channel-status-local" => channel::cmd_channel_status_local(),
        "channel-finalize" => channel::cmd_channel_finalize(),
        "gateway" => {
            let state_dir = get(&flags, "state-dir")
                .map(PathBuf::from)
                .unwrap_or_else(experiment::crate_dir);
            channel::run_gateway(&state_dir)
        }
        "discover" => {
        ...
        _ => {
            ...
            eprintln!("run: cargo run -p mutagen-exp-0014 -- help");
