...
//!      (0012 mechanism, `src/stage_freeze.rs`):
...
//!
//! No-restart rule (0012 critical): a live stage whose output already
...
use crate::model::{
    self, EPISODE_TIME_LIMIT_MS, ModelClient, ModelError, REGISTERED_ENDPOINT, REQUEST_TIMEOUT,
};
use crate::mutation::{
...
const RAW_DISCOVERY_TRAJ: &str = "0012-discovery-trajectories.jsonl";
const RAW_DISCOVERY_SUMMARY: &str = "0012-discovery-summary.json";
const RAW_MUTATION_GENERATION: &str = "0012-mutation-generation.json";
const RAW_SELECTION_TRAJ: &str = "0012-selection-trajectories.jsonl";
const RAW_SELECTION_SUMMARY: &str = "0012-selection-summary.json";
const RAW_PROMOTION_TRAJ: &str = "0012-promotion-trajectories.jsonl";
const RAW_PROMOTION_SUMMARY: &str = "0012-promotion-summary.json";
...
pub fn load_tasks(path: &Path) -> Result<TaskFile, String> {
    ...
    if file.experiment_id != "0012" {
        return Err(format!(
            "task registry experiment_id {:?} does not match this experiment (0012)",
            file.experiment_id
        ));
    }
    file.validate()
...
/// (0012 mechanism #2). Any violation ⇒ the next live stage refuses to
...
/// No-restart rule (0012 critical): a live stage whose output file
...
/// environment. The registered hard limits (turns, calls) come from
/// the design manifest; the per-episode time budget is the registered
/// constant in `src/model.rs`. Never panics on model/tool failure: all
...
fn execute_episode(
    ...
    let budget = Duration::from_millis(EPISODE_TIME_LIMIT_MS);
        ...
        |messages, tool_specs| {
            ...
            if remaining.is_zero() {
                return Err(ModelError::Http(
                    "episode time limit exceeded (registered 10-minute limit)".into(),
                ));
            }
            client.complete(
                messages,
...
/// 0012 deliberately has no `--family` / `--task` live filter: any
...
pub fn cmd_discover(args: DiscoverArgs) -> Result<(), String> {
    ...
    println!(
        "NEXT — commit B artifacts + manifest together, then run Stage C:\n  git add {t} {s} {mi} {mf}\n  git commit -m \"exp: freeze 0012 discovery evidence\"\n  cargo run -p mutagen-exp-0012 -- generate",
...
pub fn cmd_generate(args: GenerateArgs) -> Result<(), String> {
    ...
    println!(
        "NEXT — commit C artifacts + manifest together, then run Stage D:\n  git add {} {}\n  git commit -m \"exp: freeze 0012 generated candidate pool\"\n  cargo run -p mutagen-exp-0012 -- select",
        raw_rel_path(RAW_MUTATION_GENERATION),
...
pub fn cmd_select(args: SelectArgs) -> Result<(), String> {
    ...
    println!(
        "NEXT — commit D artifacts + manifest together:\n  git add {} {} {}\n  git commit -m \"exp: freeze 0012 selected candidate before promotion\"",
        raw_rel_path(RAW_SELECTION_TRAJ),
        raw_rel_path(RAW_SELECTION_SUMMARY),
        crate_rel_path(SELECTED_CANDIDATE),
    );
...
pub fn cmd_init_run(args: InitRunArgs) -> Result<(), String> {
    ...
    let manifest = freeze::build_manifest(&root, &commit, "0012-r1", "0012")?;
...
pub fn print_a0_commands(commit_message: &str) {
    ...
    println!(
        "  git rev-parse HEAD   # → pass to: cargo run -p mutagen-exp-0012 -- init-run --commit <SHA>"
    );
}
...
/// `tool_choice = "auto"` (one tool call per turn, frozen in 0012).
...
pub fn cmd_preflight(manifest: Option<&Path>) -> Result<(), String> {
    println!("== Experiment 0012 preflight (network-free) ==");
    let mut fail = false;
    ...
    match (load_design(), load_tasks(&crate_dir().join("tasks.json"))) {
        (Ok(design), Ok(file)) => {
            ...
            // Fresh task split + V12 literal split-disjointness.
            ...
            report(
                "fresh task split (12 tasks: 3 discovery / 3 selection / 6 promotion, split-disjoint V12 literals)",
            ...
            .and_then(|v| v.as_array_mut())
        {
            if let Some(st) = eps.first_mut().and_then(|e| e.get_mut("final_state")) {
                if let Some(o) = st.as_object_mut() {
                    o.insert("x".into(), Value::String("V12_SA_0".into())); // selection literal
                }
            }
        }
        let v = trace::verify_discovery(
