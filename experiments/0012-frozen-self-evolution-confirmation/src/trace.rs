...
//! Experiment 0012 (the frozen end-to-end self-evolution
//! confirmation).
//!
...
pub struct Design {
    ...
    /// fallback model. The endpoint URL and the per-episode time budget
    /// are registered named constants in `src/model.rs` (neither is a
    /// count or a gate), not design fields.
    pub model: String,
}
...
impl Design {
    ...
    /// wrapped — the 0012 registered schedule has five conditions, so
...
pub struct RunIdentity {
    /// The registered run identity (design `run_id`), e.g. "0012-r1".
    pub run_id: String,
...
fn verify_record_core<R: RecordCoreLike>(
    ...
    if record.timing().wall_time_ms > crate::model::EPISODE_TIME_LIMIT_MS {
        errors.push(format!(
            "{who}: wall time {} ms exceeds the registered {} ms episode time limit",
            record.timing().wall_time_ms,
            crate::model::EPISODE_TIME_LIMIT_MS
        ));
    }
...
pub fn build_synthetic_artifacts() -> SyntheticArtifacts {
    ...
    let design = Design::load(&dir.join("design.json")).expect("0012 design manifest loads");
