...
//! registered model exist for this experiment (the design manifest
//! carries the model name; the endpoint URL and the per-episode time
//! budget are the named, preflight-reported constants in this module —
//! there is no fallback model and no second network route), so there
//! is no `ModelProvider` trait and no abstraction — exactly one
...
//! command does not exist in 0012).
...
/// Per-request timeout ceiling. Local Qwen generation can be slow;
/// this is a generous safety bound, not a tuning knob. Callers may
/// pass a SHORTER per-request timeout (the episode's remaining time
/// budget) to enforce the registered per-episode time limit exactly.
pub const REQUEST_TIMEOUT: Duration = Duration::from_secs(600);

/// Registered per-episode hard time budget in milliseconds (10
/// minutes; the carried-over 0011 value). Named constant — reported by
/// `preflight`, checked by the record verifier; the design manifest
/// does not carry it.
pub const EPISODE_TIME_LIMIT_MS: u64 = 600_000;
...
impl ModelClient {
    ...
    /// auto`, `parallel_tool_calls = false`. `timeout` is the
    /// per-request ceiling: callers pass the EPISODE's remaining time
    /// budget (clamped to [`REQUEST_TIMEOUT`]) so the registered
    /// per-episode time limit holds even for a single long generation.
    /// Returns the kernel-relevant parsed response.
    pub fn complete(
        &self,
        messages: &[Message],
        tools: &[ToolSpec],
